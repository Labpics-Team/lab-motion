...
import { SHEET_OBSERVABLE_V3 } from './sheet-observable-v3-preregistration.mjs';
...
function invariant(condition, message) {
  if (!condition) throw new Error(`PROFILE-01 sheet-observable-v3: ${message}`);
}
...
function sha256(value) {
  return createHash().update(typeof value === 'string' ? value : JSON.stringify(value)).digest('hex');
}
...
function validateFrozenContract(contract) {
  ...
  const inputVelocity = (up.y - lastMove.y) / dt;
  invariant(
    inputVelocity === contract.expected.releaseInputVelocityPxPerSec,
    `release input velocity drifted (${inputVelocity})`,
  );
  invariant(up.atMs < interrupt.atMs && interrupt.atMs < terminal.atMs, 'release/interrupt/terminal order drifted');
  invariant(contract.frameStepMs > 0 && Number.isFinite(contract.frameStepMs), 'frameStepMs must be positive and finite');
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      if (pathname === '/') {
        ...
        response.end('<!doctype html><meta charset="utf-8"><title>PROFILE-01 sheet observable v3</title>');
        return;
      }
    ...
    try {
      ...
      const result = await page.evaluate(async ({ behaviorsUrl, frozen }) => {
        ...
        const assertMovement = (before, after, dtMs, direction) => {
          if (!Number.isFinite(before) || !Number.isFinite(after) || !Number.isFinite(dtMs) || dtMs <= 0) {
            throw new Error('movement oracle received non-finite input');
          }
          const delta = after - before;
          const velocity = delta / (dtMs / 1000);
          if (delta === 0 || !Number.isFinite(velocity) || Math.sign(delta) !== direction) {
        ...
        const assertTerminal = (state, pending, expected) => {
          if (
            ...
            throw new Error(`terminal-at-deadline law failed: ${JSON.stringify({ state, pending, expected })}`);
          }
        };
          ...
          return {
            ...
            stepTo(timestamp) {
              if (!Number.isFinite(timestamp) || timestamp < now) {
                throw new Error(`virtual clock moved backwards (${now} -> ${timestamp})`);
              }
              now = timestamp;
              const batch = queue;
              queue = [];
            ...
            advanceTo(deadline, stepMs) {
              while (queue.length > 0 && now < deadline) {
                this.stepTo(Math.min(deadline, now + stepMs));
              }
            },
        ...
        try {
          ...
          // Первый runner tick устанавливает timestamp, второй обязан дать независимо
          // наблюдаемое движение. Скорость выводится только из public value delta.
          ...
          if (atInterrupt.phase !== 'release') {
            throw new Error(`release settled before frozen interruption: ${JSON.stringify(atInterrupt)}`);
          }
          if (atInterrupt.value === release.value) {
            throw new Error(`no observable movement by interruption: ${JSON.stringify({ release, atInterrupt })}`);
          }
          ...
          // Deadline is a hard boundary: execute only callbacks timestamped <= deadline.
          // No drain, retry or post-deadline frame may turn a miss into PASS.
...
export async function probeSheetObservableV3({ baselineRoot, harnessRevision, contract = SHEET_OBSERVABLE_V3 }) {
...
async function main() {
  ...
  const receipt = await probeSheetObservableV3({ baselineRoot, harnessRevision });
