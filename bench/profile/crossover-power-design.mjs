...
import { finalizeCrossoverPilotReceipt } from './crossover-pilot-core.mjs';
import { pairedLogReceiptSha256 } from './paired-log-pilot-core.mjs';
import { estimateScenePower } from './power-design.mjs';

function invariant(condition, message) {
  if (!condition) throw new Error(`PROFILE-01 symmetric-crossover power: ${message}`);
}
...
export function deriveCrossoverPoweredDesign(pilot, inventory, calibration, generatedAt = new Date().toISOString()) {
  validateCrossoverPreregistration();
  const acceptedPilot = finalizeCrossoverPilotReceipt(pilot);
  invariant(acceptedPilot.inventorySha256 === pairedLogReceiptSha256(inventory), 'inventory content address does not match pilot');
  invariant(calibration?.profileId === PROFILE_PREREGISTRATION.profileId, 'calibration profile drifted');
  invariant(calibration?.baselineRevision === PROFILE_PREREGISTRATION.baseline.revision, 'calibration baseline drifted');
  invariant(calibration?.status === 'PASS', 'calibration is not PASS');
...
async function main() {
  ...
  const design = deriveCrossoverPoweredDesign(pilot, inventory, calibration);
