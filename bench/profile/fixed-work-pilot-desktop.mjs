...
async function baselineBundle(baselineDir) {
  ...
  const result = await build({
    stdin: {
      ...
      sourcefile: 'profile-fixed-work-entry.mjs',
    },
    bundle: true,
    write: false,
    format: 'iife',
  ...
  await page.evaluate(() => {
    ...
    const runDirectManipulation = (copies) => {
      ...
      own += timedRegion(() => {
        for (let index = 0; index < copies; index++) {
          drags[index] = globalThis.__lm.createDrag({
            ...
            matchMedia: () => ({ matches: false }),
            // Callback dispatch is library-owned, consumer rendering is not.
            onStep: () => {},
          });
        }
      });
      ...
      for (let index = 0; index < copies; index++) {
        ...
        // Render after owner timing to preserve the corrected attribution boundary.
    ...
    globalThis.__labMotionFixedWork = {
...
async function measurePacket(page, request) {
  return page.evaluate(
    ([sceneId, copies, logicalUnits, workMultiplier]) => globalThis.__labMotionFixedWork.measurePacket(sceneId, copies, logicalUnits, workMultiplier),
...
async function measureEngine(engine, browserType, bundle, inventory) {
  ...
  try {
    ...
    for (let sceneIndex = 0; sceneIndex < DESIGN.sceneIds.length; sceneIndex++) {
      ...
      const raw = await acquireFixedWorkControls(
...
async function main() {
  ...
  const outputPath = process.env.PROFILE_FIXED_WORK_OUTPUT;
  const harnessRevision = process.env.PROFILE_HARNESS_REVISION ?? '';
  const preregRevision = process.env.PROFILE_FIXED_WORK_PREREG_REVISION ?? '';
  ...
  try {
    ...
    const failureReceipt = {
      ...
      failure: {
        ...
        evidence: error instanceof FixedWorkResolutionFailure ? error.evidence : null,
      },
    };
  ...
  const raw = buildFixedWorkPilotReceipt({ inventory, harnessRevision, preregRevision, cells });
  await writeFile(outputPath, `${JSON.stringify(raw, null, 2)}\n`, 'utf8');
  const pilot = finalizeFixedWorkPilotReceipt(raw);
  await writeFile(outputPath, `${JSON.stringify(pilot, null, 2)}\n`, 'utf8');
  process.stdout.write(`${JSON.stringify({
    status: 'PASS',
    harnessKind: FIXED_WORK_HARNESS_KIND,
    designId: DESIGN.id,
    candidateSamples: 0,
    sha256: pairedLogReceiptSha256(pilot),
    cells: pilot.cells.map((cell) => ({
      id: cell.id,
      scenes: cell.scenes.map((scene) => ({ id: scene.id, aa: scene.aa, deliberate2x: scene.deliberate2x })),
    })),
  }, null, 2)}\n`);
}
