...
import { finalizeProcessResetPilotReceipt } from './process-reset-pilot-core.mjs';
...
function invariant(condition, message) {
  if (!condition) throw new Error(`PROFILE-01 process-reset power: ${message}`);
}
...
function sceneSeed(cellIndex, sceneIndex) {
  return (
    PROFILE_PREREGISTRATION.statistics.bootstrapSeed ^
...
export function deriveProcessResetPoweredDesign(pilot, inventory, calibration, generatedAt = new Date().toISOString()) {
  validateProcessResetPreregistration();
  validateCalibrationReceipt(calibration);
  const acceptedPilot = finalizeProcessResetPilotReceipt(pilot);
  ...
  const cells = acceptedPilot.cells.map((cell, cellIndex) => {
    const noise = cell.scenes.map((scene, sceneIndex) => {
      const initial = estimateScenePower(
        scene,
        DESIGN.minimumIndependentBlocks,
        sceneSeed(cellIndex, sceneIndex),
    ...
    for (let blocks = DESIGN.minimumIndependentBlocks; blocks <= DESIGN.maximumIndependentBlocks; blocks++) {
      const scenePowers = noise.map(({ id, sigma }) => ({
        id,
        noiseSigmaUpper95: sigma,
        estimatedPower: powerFromSigma(sigma, blocks, id),
      }));
    ...
    const previousIndependentBlocks = chosen.blocks === DESIGN.minimumIndependentBlocks ? null : chosen.blocks - 1;
    const previousEstimatedPower = previousIndependentBlocks === null
      ? null
      : Math.min(...noise.map(({ id, sigma }) => powerFromSigma(sigma, previousIndependentBlocks, id)));
  ...
  return {
    ...
    processIsolation: acceptedPilot.processIsolation,
    cells,
  };
}
...
async function main() {
  ...
  const design = deriveProcessResetPoweredDesign(pilot, inventory, calibration);
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    candidateSamples: 0,
    sha256: pairedLogReceiptSha256(design),
    processIsolation: design.processIsolation,
    cells: design.cells.map(({ id, chosenIndependentBlocks, previousIndependentBlocks, previousEstimatedPower, estimatedPower, status }) => ({
      id,
      chosenIndependentBlocks,
      previousIndependentBlocks,
      previousEstimatedPower,
      estimatedPower,
      status,
