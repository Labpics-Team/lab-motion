...
import { ASYMMETRIC_TIMING_PREREGISTRATION as DESIGN } from './asymmetric-preregistration.mjs';
import { validateDesktopInventory } from './validate.mjs';

function invariant(condition, message) {
  if (!condition) throw new Error(`PROFILE-01 asymmetric pilot: ${message}`);
}

class AsymmetricPilotFailure extends Error {
  constructor(message, evidence) {
    super(`PROFILE-01 asymmetric pilot: ${message}`);
    this.name = 'AsymmetricPilotFailure';
...
function admittedDifferential(run, sample, floorMs, label) {
  ...
  invariant(Number.isFinite(sample.estimateMs) && sample.estimateMs > 0, `${label}: normalized differential is not positive`);
  return {
    run,
...
export async function chooseArmRepeats(measureArm, arm, options = {}) {
  ...
  for (const repeats of candidates) {
    ...
    if (!holdout.every((elapsed) => elapsed >= floorMs)) {
      throw new AsymmetricPilotFailure(`${arm} holdout failed at ${repeats} repeats; same-pilot escalation is forbidden`, {
        stage: 'selector-holdout', arm, floorMs, candidates: [...candidates], attempts, repeats, holdout,
      });
    }
    return { arm, repeats, discovery, holdout };
  }
  throw new AsymmetricPilotFailure(`no preregistered repeat count resolves ${arm} arm above ${floorMs}ms`, {
    stage: 'selector-exhausted', arm, floorMs, candidates: [...candidates], attempts,
  });
}

export async function acquireAsymmetricControls(measureDifferential, selectedRepeats, options = {}) {
...
async function measureArmPair(page, sceneId, selectedRepeats, factor, motionFirst) {
  ...
  return {
    ...
    motionNormalizedMs,
    controlNormalizedMs,
    estimateMs: motionNormalizedMs - controlNormalizedMs,
    factor,
  };
}
  ...
  try {
    ...
    for (let sceneIndex = 0; sceneIndex < DESIGN.sceneIds.length; sceneIndex++) {
      ...
      try {
        ...
        if (error instanceof AsymmetricPilotFailure) {
          throw new AsymmetricPilotFailure(error.message.replace(/^PROFILE-01 asymmetric pilot: /u, ''), {
...
async function main() {
  ...
  const outputPath = process.env.PROFILE_ASYMMETRIC_OUTPUT;
  const harnessRevision = process.env.PROFILE_HARNESS_REVISION ?? '';
  const preregRevision = process.env.PROFILE_ASYMMETRIC_PREREG_REVISION ?? '';
