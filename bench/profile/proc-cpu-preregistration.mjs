...
export const PROC_CPU_PREREGISTRATION = frozen({
  schemaVersion: 1,
  id: 'linux-browser-tree-schedstat-cpu-v1',
  node: 'PROFILE-01',
  ...
  premiseChange: frozen({
    ...
    to: 'kernel scheduler-accounted CPU runtime delta for the stable browser process tree around each complete semantic arm',
    rationale: 'schedstat accounts actual on-CPU runtime instead of elapsed phase/order jitter and timer-region accumulation; it therefore changes the measured representation rather than retuning the exhausted owner-time family',
  ...
  measurement: frozen({
    kind: 'linux-proc-browser-tree-schedstat-cpu-ms-v1',
    ...
    platformRule: 'Linux /proc is mandatory; /proc/<pid>/schedstat field 1 is interpreted as cumulative on-CPU runtime nanoseconds and must be readable, positive and monotonic',
    ...
    treeRule: 'browser-server root plus every recursive /proc child is snapshotted immediately before and after each formal arm; exact pid+starttime identity set must remain unchanged or the pilot fails closed',
    ...
    attributionBoundary: 'sample is total scheduler CPU consumed by the stable browser process tree during the complete semantic arm; Node runner, launch, setup before warmup, warmup and teardown are excluded; no performance.now owner timing contributes to the sample',
...
export function validateProcCpuPreregistration(design = PROC_CPU_PREREGISTRATION) {
  ...
  invariant(design.measurement.kind === 'linux-proc-browser-tree-schedstat-cpu-ms-v1', 'measurement representation drifted');
  ...
  invariant(/schedstat field 1/.test(design.measurement.platformRule), 'scheduler clock owner missing');
  invariant(/exact pid\+starttime identity set/.test(design.measurement.treeRule), 'process-tree stability rule missing');
