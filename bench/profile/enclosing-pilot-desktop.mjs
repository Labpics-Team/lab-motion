...
import { ENCLOSING_TIMING_PREREGISTRATION as DESIGN } from './enclosing-preregistration.mjs';
import { validateDesktopInventory } from './validate.mjs';

function invariant(condition, message) {
  if (!condition) throw new Error(`PROFILE-01 enclosing pilot: ${message}`);
}

class EnclosingPilotFailure extends Error {
  constructor(message, evidence) {
    super(`PROFILE-01 enclosing pilot: ${message}`);
    this.name = 'EnclosingPilotFailure';
...
function admittedEnclosingWall(run, sample, floorMs, label) {
  armFloor(sample, floorMs, label);
  invariant(Number.isSafeInteger(sample.motionRepeats) && sample.motionRepeats > 0, `${label}: motion repeat binding missing`);
  invariant(Number.isSafeInteger(sample.controlRepeats) && sample.controlRepeats > 0, `${label}: control repeat binding missing`);
  invariant(Number.isFinite(sample.motionNormalizedMs) && sample.motionNormalizedMs > 0, `${label}: direct enclosing motion wall-time is unresolved`);
  invariant(Number.isFinite(sample.controlNormalizedMs) && sample.controlNormalizedMs >= 0, `${label}: raw control wall-time is unresolved`);
  return {
    run,
    samples: [sample.motionNormalizedMs],
    rawControlMs: sample.controlNormalizedMs,
    walls: { motionMs: sample.motionWallMs, controlMs: sample.controlWallMs },
    repeats: { motion: sample.motionRepeats, control: sample.controlRepeats },
    semantic: true,
  };
}

export async function chooseArmRepeats(measureArm, arm, options = {}) {
  ...
  invariant(arm === 'motion' || arm === 'control', 'selector arm must be motion or control');
  ...
  for (const repeats of candidates) {
    ...
    for (let probe = 0; probe < discoveryProbeCount; probe++) {
      const elapsed = await measureArm(repeats);
      invariant(Number.isFinite(elapsed) && elapsed >= 0, `${arm} discovery probe returned invalid wall-time ${elapsed}`);
      discovery.push(elapsed);
    }
    attempts.push({ repeats, discovery });
    if (!discovery.every((elapsed) => elapsed >= floorMs)) continue;

    const holdout = [];
    for (let probe = 0; probe < holdoutProbeCount; probe++) {
      const elapsed = await measureArm(repeats);
      invariant(Number.isFinite(elapsed) && elapsed >= 0, `${arm} holdout probe returned invalid wall-time ${elapsed}`);
      holdout.push(elapsed);
    }
    if (!holdout.every((elapsed) => elapsed >= floorMs)) {
      throw new EnclosingPilotFailure(`${arm} holdout failed at ${repeats} repeats; same-pilot escalation is forbidden`, {
        stage: 'selector-holdout', arm, floorMs, candidates: [...candidates], attempts, repeats, holdout,
      });
    }
    return { arm, repeats, discovery, holdout };
  }
  throw new EnclosingPilotFailure(`no preregistered repeat count resolves ${arm} arm above ${floorMs}ms`, {
    stage: 'selector-exhausted', arm, floorMs, candidates: [...candidates], attempts,
  });
}

export async function acquireEnclosingControls(measureDifferential, selectedRepeats, options = {}) {
  ...
  invariant(selectedRepeats && Number.isSafeInteger(selectedRepeats.motion) && selectedRepeats.motion > 0, 'motion repeat count invalid');
  invariant(Number.isSafeInteger(selectedRepeats.control) && selectedRepeats.control > 0, 'control repeat count invalid');
  ...
  for (let run = 0; run < runBlocks; run++) {
    ...
    let b;
    if (nextOrder()) {
      b = await measureDifferential(selectedRepeats, 1);
      a = await measureDifferential(selectedRepeats, 1);
    } else {
      a = await measureDifferential(selectedRepeats, 1);
      b = await measureDifferential(selectedRepeats, 1);
    }
    aa.a.push(admittedEnclosingWall(run, a, floorMs, `run ${run}/aa-a`));
    aa.b.push(admittedEnclosingWall(run, b, floorMs, `run ${run}/aa-b`));

    let single;
    let doubled;
    if (nextOrder()) {
      doubled = await measureDifferential(selectedRepeats, 2);
      single = await measureDifferential(selectedRepeats, 1);
    } else {
      single = await measureDifferential(selectedRepeats, 1);
      doubled = await measureDifferential(selectedRepeats, 2);
    }
    deliberate2x.single.push(admittedEnclosingWall(run, single, floorMs, `run ${run}/single`));
    deliberate2x.doubled.push(admittedEnclosingWall(run, doubled, floorMs, `run ${run}/doubled`));
  }
...
async function measureSingleArm(page, sceneId, arm, repeats) {
  return page.evaluate(([id, selectedArm, copies, count]) => (
    globalThis.__labMotionDifferential.measureArm(id, selectedArm, copies, count)
  ), [sceneId, arm, DESIGN.liveBatchCalls, repeats]);
}

async function measureArmPair(page, sceneId, selectedRepeats, factor, motionFirst) {
  const motionEffectiveRepeats = selectedRepeats.motion * factor;
  const controlEffectiveRepeats = selectedRepeats.control * factor;
  let motionWallMs;
  let controlWallMs;
  if (motionFirst) {
    motionWallMs = await measureSingleArm(page, sceneId, 'motion', motionEffectiveRepeats);
    controlWallMs = await measureSingleArm(page, sceneId, 'control', controlEffectiveRepeats);
  } else {
    controlWallMs = await measureSingleArm(page, sceneId, 'control', controlEffectiveRepeats);
    motionWallMs = await measureSingleArm(page, sceneId, 'motion', motionEffectiveRepeats);
  }
  const motionNormalizedMs = motionWallMs / selectedRepeats.motion;
  const controlNormalizedMs = controlWallMs / selectedRepeats.control;
  return {
    motionWallMs,
    controlWallMs,
    motionRepeats: selectedRepeats.motion,
    controlRepeats: selectedRepeats.control,
    motionNormalizedMs,
    controlNormalizedMs,
    factor,
  };
...
async function measureEngine(engine, browserType, bundle, inventory) {
  ...
  try {
    ...
    for (let sceneIndex = 0; sceneIndex < DESIGN.sceneIds.length; sceneIndex++) {
      ...
      let controlSelector;
      try {
        motionSelector = await chooseArmRepeats((repeats) => measureSingleArm(page, sceneId, 'motion', repeats), 'motion');
        controlSelector = await chooseArmRepeats((repeats) => measureSingleArm(page, sceneId, 'control', repeats), 'control');
      } catch (error) {
        if (error instanceof EnclosingPilotFailure) {
          throw new EnclosingPilotFailure(error.message.replace(/^PROFILE-01 enclosing pilot: /u, ''), {
      ...
      let raw;
      try {
        raw = await acquireEnclosingControls(measureDifferential, selectedRepeats, {
          orderSeed: PROFILE_PREREGISTRATION.statistics.orderSeed ^ Math.imul(sceneIndex + 1, 0x119de1f3),
        });
      } catch (error) {
        if (error instanceof EnclosingPilotFailure) throw error;
        throw new EnclosingPilotFailure(error instanceof Error ? error.message.replace(/^PROFILE-01 enclosing pilot: /u, '') : String(error), {
      ...
      scenes.push({
        id: sceneId,
        selectedRepeats,
        selector: { motion: motionSelector, control: controlSelector },
        raw,
        aa,
        deliberate2x,
      });
...
function validateCalibration(cells) {
  ...
  for (const cell of cells) for (const scene of cell.scenes) {
    invariant(scene.aa.lower95 >= aaLow && scene.aa.upper95 <= aaHigh, `${cell.id}/${scene.id}: A/A enclosing wall ratio escaped [${aaLow}, ${aaHigh}]`);
    invariant(scene.deliberate2x.lower95 >= PROFILE_PREREGISTRATION.calibration.deliberateWorkDetectedLower95Min, `${cell.id}/${scene.id}: deliberate-2x enclosing wall unresolved`);
    invariant(scene.deliberate2x.lower95 >= factorTwoLow && scene.deliberate2x.upper95 <= factorTwoHigh, `${cell.id}/${scene.id}: factor-2 enclosing wall scale escaped [${factorTwoLow}, ${factorTwoHigh}]`);
  }
}
...
async function main() {
  ...
  const outputPath = process.env.PROFILE_ENCLOSING_OUTPUT;
  const harnessRevision = process.env.PROFILE_HARNESS_REVISION ?? '';
  const preregRevision = process.env.PROFILE_ENCLOSING_PREREG_REVISION ?? '';
