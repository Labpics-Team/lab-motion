...
async function baselineBundle(baselineDir) {
  ...
  const result = await build({
    stdin: {
      ...
      sourcefile: 'profile-proc-cpu-entry.mjs',
    },
    bundle: true,
    write: false,
    format: 'iife',
...
async function installHarness(page, bundle) {
  ...
  await page.evaluate(() => {
    const timedRegion = (fn) => { fn(); return 0; };
...
function assertPilotWallBudget(pilotStartedAt) {
  ...
  if (elapsed > DESIGN.measurement.maximumPilotWallMs) {
    throw new ProcCpuFailure('pilot exceeded whole-pilot wall bound', {
...
async function measureProcCpuPair(browserType, engine, bundle, expectedVersion, request, pairOrdinal, pilotStartedAt) {
  ...
  try {
    ...
    for (let position = 0; position < request.arms.length; position++) {
      ...
      const warmup = await page.evaluate(
        ([sceneId, batchCalls, warmupLogicalUnits]) => globalThis.__labMotionProcCpu.warmup(sceneId, batchCalls, warmupLogicalUnits),
...
async function measureEngine(engine, browserType, bundle, inventory, pilotStartedAt) {
  ...
  let pairOrdinal = 0;
  const measure = (request) => measureProcCpuPair(
    browserType,
    engine,
    bundle,
    bound.version,
  ...
  for (let sceneIndex = 0; sceneIndex < DESIGN.sceneIds.length; sceneIndex++) {
    ...
    const raw = await acquireProcCpuControls(measure, sceneId, {
...
async function main() {
  ...
  const outputPath = process.env.PROFILE_PROC_CPU_OUTPUT;
  const harnessRevision = process.env.PROFILE_HARNESS_REVISION ?? '';
  const preregRevision = process.env.PROFILE_PROC_CPU_PREREG_REVISION ?? '';
  ...
  try {
    ...
    const receipt = buildProcCpuPilotReceipt({
      inventory,
      calibration,
      preregRevision,
      harnessRevision,
    ...
    const accepted = finalizeProcCpuPilotReceipt(receipt);
    ...
    const failureReceipt = {
      ...
      failure: {
        ...
        evidence: error instanceof ProcCpuFailure ? error.evidence : null,
      },
    };
