...
} from './fresh-paired-block-pilot-core.mjs';
...
export const FRESH_PAIRED_BLOCK_HARNESS_KIND = 'fresh-browser-process-paired-run-block-owner-ms-v1';

function invariant(condition, message) {
  if (!condition) throw new Error(`PROFILE-01 fresh-paired-block desktop: ${message}`);
}
...
async function baselineBundle(baselineDir) {
  ...
  const result = await build({
    stdin: {
      ...
      sourcefile: 'profile-fresh-paired-block-entry.mjs',
    },
    bundle: true,
    write: false,
    format: 'iife',
...
async function installHarness(page, bundle) {
  ...
  await page.evaluate(() => {
    ...
    globalThis.__labMotionFreshPairedBlock = {
...
function assertPilotWallBudget(pilotStartedAt) {
  ...
  if (elapsed > DESIGN.measurement.maximumPilotWallMs) {
    throw new FreshPairedBlockFailure('pilot exceeded whole-pilot wall bound', {
  ...
  try {
    ...
    for (let position = 0; position < request.arms.length; position++) {
      ...
      const warmup = await page.evaluate(
        ([sceneId, batchCalls, warmupLogicalUnits]) => globalThis.__labMotionFreshPairedBlock.warmup(sceneId, batchCalls, warmupLogicalUnits),
      ...
      const measured = await page.evaluate(
        ([sceneId, batchCalls, logicalUnits, workMultiplier]) => globalThis.__labMotionFreshPairedBlock.measure(sceneId, batchCalls, logicalUnits, workMultiplier),
...
async function measureEngine(engine, browserType, bundle, inventory, pilotStartedAt) {
  ...
  let pairOrdinal = 0;
  const measure = (request) => measureFreshPair(
    browserType,
    engine,
    bundle,
    bound.version,
  ...
  for (let sceneIndex = 0; sceneIndex < DESIGN.sceneIds.length; sceneIndex++) {
    ...
    const raw = await acquireFreshPairedControls(measure, sceneId, {
...
async function main() {
  ...
  const outputPath = process.env.PROFILE_FRESH_PAIRED_BLOCK_OUTPUT;
  const harnessRevision = process.env.PROFILE_HARNESS_REVISION ?? '';
  const preregRevision = process.env.PROFILE_FRESH_PAIRED_BLOCK_PREREG_REVISION ?? '';
  ...
  try {
    ...
    const receipt = buildFreshPairedPilotReceipt({
      inventory,
      calibration,
      preregRevision,
      harnessRevision,
    ...
    const failureReceipt = {
      ...
      failure: {
        ...
        evidence: error instanceof FreshPairedBlockFailure ? error.evidence : null,
      },
    };
