...
export const REFERENCE_NORMALIZED_PREREGISTRATION = frozen({
  ...
  premiseChange: frozen({
    ...
    to: 'fixed semantic owner work normalized by a validated same-engine calibration workload measured immediately before and after each packet',
    rationale: 'the surviving direct-cell variance can arise from multiplicative browser/runner speed changes that first-order order balancing cannot remove; a local bracketing covariate measures that nuisance scale without changing product work',
  ...
  measurement: frozen({
    kind: 'fixed-semantic-work-calibration-bracket-normalized-v1',
