...
} from './budget-pilot-core.mjs';
import { validateDesktopInventory } from './validate.mjs';

export const BUDGET_HARNESS_KIND = 'owned-time-budget-desktop-v1';

function invariant(condition, message) {
  if (!condition) throw new Error(`PROFILE-01 owned-time desktop: ${message}`);
}
...
async function baselineBundle(baselineDir) {
  ...
  const result = await build({
    stdin: {
      ...
      sourcefile: 'profile-owned-time-entry.mjs',
    },
    bundle: true,
    write: false,
    format: 'iife',
...
async function installHarness(page, bundle) {
  ...
  await page.evaluate(() => {
    ...
    globalThis.__labMotionOwnedTime = {
...
async function measurePhysicalExecution(page, sceneId) {
  return page.evaluate(([id, copies]) => globalThis.__labMotionOwnedTime.measure(id, copies), [sceneId, DESIGN.liveBatchCalls]);
}
...
async function measureEngine(engine, browserType, bundle, inventory) {
  ...
  try {
    ...
    for (let sceneIndex = 0; sceneIndex < DESIGN.sceneIds.length; sceneIndex++) {
      ...
      const raw = await acquireOwnedTimeControls(
...
async function main() {
  ...
  const outputPath = process.env.PROFILE_BUDGET_OUTPUT;
  const harnessRevision = process.env.PROFILE_HARNESS_REVISION ?? '';
  const preregRevision = process.env.PROFILE_BUDGET_PREREG_REVISION ?? '';
  ...
  try {
    ...
    const failureReceipt = {
      ...
      failure: {
        ...
        evidence: error instanceof OwnedTimeBudgetFailure ? error.evidence : null,
      },
    };
  ...
  const raw = buildOwnedTimePilotReceipt({ inventory, harnessRevision, preregRevision, cells });
  await writeFile(outputPath, `${JSON.stringify(raw, null, 2)}\n`, 'utf8');
  const pilot = finalizeOwnedTimePilotReceipt(raw);
  await writeFile(outputPath, `${JSON.stringify(pilot, null, 2)}\n`, 'utf8');
  process.stdout.write(`${JSON.stringify({
    status: 'PASS',
    harnessKind: BUDGET_HARNESS_KIND,
    designId: DESIGN.id,
    candidateSamples: 0,
    sha256: budgetReceiptSha256(pilot),
