...
import {
  ...
  pairedLogReceiptSha256,
  buildPairedLogPilotReceipt,
  finalizePairedLogPilotReceipt,
} from './paired-log-pilot-core.mjs';
import { validateDesktopInventory } from './validate.mjs';

export const PAIRED_LOG_HARNESS_KIND = 'paired-log-owner-budget-desktop-v2';

function invariant(condition, message) {
  if (!condition) throw new Error(`PROFILE-01 paired-log desktop: ${message}`);
}
...
async function baselineBundle(baselineDir) {
  ...
  const result = await build({
    stdin: {
      ...
      sourcefile: 'profile-paired-log-entry.mjs',
    },
    bundle: true,
    write: false,
    format: 'iife',
  ...
  await page.evaluate(() => {
    ...
    globalThis.__labMotionPairedLog = {
      measure(sceneId, copies) {
        ...
        return { ownerMs, enclosingWallMs };
      },
    };
  });
}

async function measurePhysicalExecution(page, sceneId) {
  return page.evaluate(([id, copies]) => globalThis.__labMotionPairedLog.measure(id, copies), [sceneId, DESIGN.liveBatchCalls]);
}
...
async function measureEngine(engine, browserType, bundle, inventory) {
  ...
  try {
    ...
    for (let sceneIndex = 0; sceneIndex < DESIGN.sceneIds.length; sceneIndex++) {
      ...
      const raw = await acquirePairedLogControls(
        () => measurePhysicalExecution(page, sceneId),
...
async function main() {
  ...
  const outputPath = process.env.PROFILE_PAIRED_LOG_OUTPUT;
  const harnessRevision = process.env.PROFILE_HARNESS_REVISION ?? '';
  const preregRevision = process.env.PROFILE_PAIRED_LOG_PREREG_REVISION ?? '';
  ...
  try {
    ...
    const failureReceipt = {
      ...
      failure: {
        ...
        evidence: error instanceof PairedLogBudgetFailure ? error.evidence : null,
      },
    };
  ...
  const raw = buildPairedLogPilotReceipt({ inventory, harnessRevision, preregRevision, cells });
  await writeFile(outputPath, `${JSON.stringify(raw, null, 2)}\n`, 'utf8');
  const pilot = finalizePairedLogPilotReceipt(raw);
  await writeFile(outputPath, `${JSON.stringify(pilot, null, 2)}\n`, 'utf8');
  process.stdout.write(`${JSON.stringify({
    status: 'PASS',
    harnessKind: PAIRED_LOG_HARNESS_KIND,
    designId: DESIGN.id,
    candidateSamples: 0,
