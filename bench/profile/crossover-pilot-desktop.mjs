...
} from './crossover-pilot-core.mjs';
import { pairedLogReceiptSha256 } from './paired-log-pilot-core.mjs';
import { validateDesktopInventory } from './validate.mjs';

export const CROSSOVER_HARNESS_KIND = 'fixed-work-symmetric-crossover-desktop-v1';

function invariant(condition, message) {
  if (!condition) throw new Error(`PROFILE-01 symmetric-crossover desktop: ${message}`);
}
...
async function baselineBundle(baselineDir) {
  ...
  const result = await build({
    stdin: {
      ...
      sourcefile: 'profile-symmetric-crossover-entry.mjs',
    },
    bundle: true,
    write: false,
    format: 'iife',
...
async function installHarness(page, bundle) {
  ...
  await page.evaluate(() => {
    ...
    globalThis.__labMotionCrossover = {
...
async function main() {
  ...
  const outputPath = process.env.PROFILE_CROSSOVER_OUTPUT;
  const harnessRevision = process.env.PROFILE_HARNESS_REVISION ?? '';
  const preregRevision = process.env.PROFILE_CROSSOVER_PREREG_REVISION ?? '';
  ...
  try {
    ...
    const failureReceipt = {
      ...
      failure: {
        ...
        evidence: error instanceof CrossoverResolutionFailure ? error.evidence : null,
      },
    };
  ...
  process.stdout.write(`${JSON.stringify({
    status: 'PASS',
    harnessKind: CROSSOVER_HARNESS_KIND,
    designId: DESIGN.id,
    candidateSamples: 0,
    sha256: pairedLogReceiptSha256(pilot),
    cells: pilot.cells.map((cell) => ({ id: cell.id, scenes: cell.scenes.map((scene) => ({ id: scene.id, aa: scene.aa, deliberate2x: scene.deliberate2x })) })),
  }, null, 2)}\n`);
}
