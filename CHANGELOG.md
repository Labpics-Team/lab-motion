...
- **Стирание полного фасада компилятором (#240, «нулевая библиотека»):** вызов
  полного `./animate`, помеченный блочной прагмой `/* @lm-oneshot */`,
  понижается в артефакт + компактный исполнитель, а сам фасад уходит из бандла
  штатным tree-shake: приёмочный гейт фиксирует **13 624 → 1 758 B gzip
  (−87.1%)** на том же авторском вызове, причём в графе потребителя физически
  отсутствуют `animate/index`, `waapi-unit`, солвер и сегментер. Прагма —
  осознанное согласие на одноразовую семантику (нет C¹-подхвата скорости,
  residual-transform, реестра владения и контролов; старт — identity/`1`),
  всё остальное совпадает с фасадом поле в поле: кадры, кривая (тот же артефакт
  `compositor/curve`), тайминг, политика `prefers-reduced-motion` и явные стопы
  на WebKit, который не исполняет CSS `linear()`. Паритет запечатан
  C4-дифференциалом (журнал `element.animate` понижённого пути против
  фасадного). Помеченный вызов, который понизить не удалось, — **ошибка сборки
  с причиной**, а не тихий откат к 12 КБ фасада; непомеченные вызовы остаются
  полным фасадом без предупреждений. Поддержаны transform-шортхенды и
  `opacity`, числовые литералы (включая унарный минус) и пары `[from, to]`,
  опции `spring`/`delay`/`stagger`.
