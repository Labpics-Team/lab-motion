...
- **Абсолютный бюджет ошибки в единицах результата (#223):** `compileSpringPlan`
  принимает `maxValueError` — максимальную ошибку spring-реконструкции в
  единицах анимируемого свойства; effective tolerance = min(normalized,
  maxValueError/|to−from|) вычисляется один раз до кэша (эквивалентные
  authoring-входы дают один artifact), вырожденный span не делит. Новый код
  `LM170`; непосильный бюджет отвергается `LM016` (fail-closed), а не
  нарушается тихо. Горизонт компиляции продлевается для tolerance строже
  дефолтного, так что терминальный снап кривой в 1 входит в общий бюджет
  (остаток ≤ tolerance/8); артефакты с дефолтным tolerance байт-в-байт
  неизменны. Для группы каналов с общей кривой действует min по каналам.
- **Точные observable-конструкторы пружины (#230):**
  `fromPeak({ overshoot, peakTime })` — пружина из наблюдаемого первого
  перелёта и времени пика (точное обратное преобразование, один `log`, ноль
  итераций; `overshoot=1` честно означает `damping=0`, `overshoot=0`
  отклоняется новым `LM171` без epsilon-подмены);
  `fromOscillation({ period, halfLife })` — из периода затухающих колебаний и
  half-life огибающей (полюса `−α±iβ`, всегда underdamped). Оба — точные
  координатные преобразования одной модели, не пресеты, без коэрсии под
  бюджет исполнителя (#218).
- **Полный документационный суит (#91-срез):** `docs/getting-started.md`,
  справка всех 41 экспорт-субпутей (`docs/reference/`), миграции «за 15 минут»
  с Framer Motion / GSAP / Anime.js (`docs/migration/`), объяснения математики
  и архитектуры (`docs/explanations/`): closed-form пружина и полюсное
  пространство, C¹-перехват, фазовая модель main/compositor, методология
  размера.
...
- **Одноформатная поставка (ESM) и пол Node 22.12 — BREAKING для установки на
  Node 22.0–22.11:** парный CJS-артефакт (`dist/**/index.cjs`,
  `dist/**/index.d.cts`), условные ветки `import`/`require` в `exports` и
  `imports`, а также поле `main` убраны. Причина — dual-package hazard: пока
  один субпуть имел две цели, смешанный граф (ESM-код приложения импортирует
  пакет, CJS-зависимость его же требует) получал ДВА экземпляра модульного
  состояния — два кадровых цикла, два реестра `animate`, два rAF на кадр, а
  `frame.cancelAll()` из одной половины не гасил вторую. Одна цель на субпуть
  исключает это по построению. CJS-потребитель подключает пакет обычным
  `require()`: с Node 22.12 `require(esm)` работает без флага и возвращает ТОТ
  ЖЕ экземпляр, что и `import` — это проверяется новой пробой смешанного графа
  в `pack:smoke` (`require`+`import` в одном процессе → одинаковое тождество
  `frame`, 1 rAF, `cancelAll` гасит обе половины). Дополнительное следствие для
  CJS-потребителя **на TypeScript**: под `module: nodenext` нужен tsc ≥ 5.8 —
  замер на том же тарболе даёт 5.7.3 → `TS1479`, 5.8.3 и 5.9.3 → чисто; для
  ESM-потребителя ограничения нет. Публичное API не изменилось.

- **Локальная энергетическая сетка spring→`linear()` компилятора (#228):**
  глобальный worst-case пересэмплинг (плотность всей сетки по стартовой
  кривизне) заменён локальным шагом из certified-бондов кривизны на текущем
  безразмерном состоянии (спектральный √(1+4ζ²)·H; модальный |a|λs²+|b|λf²
  для ζ>1; точная критическая форма для ζ=1), состояние переходит между
  узлами замкнутой формой солвера. Замкнутая арифметика бюджета: сетка ≤
  tol/2 + RDP 3·tol/8 + сериализация ≤ tol/8 ⇒ реконструкция ≤ tolerance на
  всей непрерывной кривой. Факты на корпусе ζ∈{0.05…5}×v0∈{0,±3}×tol∈{1/400,
  1e-3, 2.5e-4}: байты строки никогда не хуже (медиана ×0.927), стопов
  никогда не больше (медиана −5), cold compile быстрее (медиана 2.16×), и
  15 точек корпуса, которые прежний worst-case гейт отвергал в `LM016`/живой
  rAF (жёсткие tolerance, большие переносимые v0), теперь компилируются в
  compositor-кривую при том же физическом капе `BASE_GRID_MAX`. Эмитируемые
  стопы при тех же входах отличаются (в пределах того же tolerance-контракта);
  `generatorVersion` compiled-origin в conformance-корпусе поднят до 2, корпус
  регенерирован тем же генератором.
- **Смена МАРШРУТА исполнения для части пружин (следствие #228, читать отдельно
  от абзаца выше):** это не только «другие стопы при тех же входах». Пружины,
  которые прежде не влезали в бюджет сетки и уходили в `LM016` либо на живой
  rAF, теперь компилируются в compositor-кривую. Приложение, полагавшееся на
  живой путь ради C¹-подхвата скорости при прерывании, после апгрейда получит
  compositor-кривую без изменений в своём коде. Обратная замена — задать более
  строгий `maxValueError`/`tolerance` либо более быструю пружину: физический
  кап `BASE_GRID_MAX` не изменился, изменилась плотность сетки под ним.
- **Фасад `./animate` стал легче anime.js v4 (шейв-порт):** семь байтовых
  компакций без изменения поведения (parseProps группирует по GroupKey на
  месте, дискретная сериализация unit/var делегирована SSOT `./value`,
...
- **Поведенческая коррекция pre-1.0 (#218):** физическая валидность пружины
  ...
  вычисляет ЛЮБУЮ физически валидную систему (включая медленные и незатухающую
  `damping=0`) — бюджет кадра-капа (`LM091`) остался границей автономных
  исполнителей (`drive`, `MotionValue`, фасад `./animate`, compositor).
  Новый публичный `validateSpringPhysics` — узкая физическая граница
  (`mass>0`, `stiffness>0`, `damping>=0`).
- **`fromBounce` / `fromVisualDuration` стали ТОЧНЫМИ (#218):** конструкторы
  возвращают точное математическое преобразование запрошенных координат
  (ω₀=2π/duration, ζ=1−bounce; точное первое касание для visualDuration) без
  скрытой коэрсии под бюджет исполнителя. `bounce=1` честно означает
  `damping=0`; представимость проверяет исполнитель на своей границе.
- **`springAsEasing`:** медленные пружины легальны (easing живёт в
  нормализованном времени); незатухающая `ζ=0` не имеет конечного горизонта и
  отклоняется новым кодом `LM167` (граница finite-easing исполнителя).
- **`springAsEasing` стала C¹-конечной (#219):** хвост запечатан Hermite-
  коррекцией `g = f + (1−f₁)(3t²−2t³) − s₁(t³−t²)` — значение и скорость
  непрерывно приходят в `(1, 0)`. Старая шкала `T=ln(100)/(ω₀·slow)` на
  критической пружине прыгала слева на ≈5.6% с наклоном ≈0.21. Горизонт —
  выведенный `normalizedSpringHorizon` (безразмерный, зависит только от ζ;
  критерий `|1−x(U)| + (4/27)|U·x′(U)| ≤` settle-допуску пакета). Коррекция
  ограничена допуском по построению; overshoot не клампится. Горячий путь:
  валидация и горизонт один раз в конструкторе, один `springUnchecked` на
  сэмпл без повторной валидации и аллокаций (≈61 нс/сэмпл).

### Fixed

- **`./nano`/compiled: чистка finish не опережает пользовательские хендлеры.**
  `commitStyles()`/`cancel()` теперь выполняются микротаском ПОСЛЕ рассылки
  события `finish` (раньше библиотечный listener, зарегистрированный первым,
  отменял анимацию до пользовательских `finish`-хендлеров — они видели
  `idle`-состояние). Guard по `playState`: `play()` из хендлера не
  затаптывается, `controls.finished` следует за новым циклом и оседает его
  `finish` (resolve) либо `cancel` (reject) — вечный pending и unhandled
  rejection отменённого реплея устранены. Паритет nano ↔ compiler/runtime
  сохранён (C4).
- **Catastrophic cancellation медленного overdamped-полюса (#226):**
  каноническая pole-space форма солвера (`α=c/m/2`, `ω²=k/m`, ветвление по
  знаку `Δ=ω²−α²`; `r_slow=−ω²/(α+√(−Δ))`). При `ζ ≳ 1e8` прежняя запись
  `−ω₀(ζ−√(ζ²−1))` теряла полюс: кривая застывала, а валидатор ЛОЖНО отвергал
  быстро оседающие жёсткие пружины через `LM091`. Ядро при этом уменьшилось
  (only-spring 792→625 B gz вместе с разделением границ #218).
