...
### Добавлено
- `StateCascade.batch` объединяет синхронную смену нескольких визуальных намерений в один итоговый target без промежуточных перезапусков исполнителя; исходное владение и последующие обычные уведомления сохраняются.
- `createStateCascade` доставляет реентрантные изменения в порядке коммитов;
  ошибки одного subscriber не обрывают остальных. `destroy()` прекращает текущую
  и отложенную доставку; stale input-getters не перезаписывают нового владельца.
  `layer.destroy()` освобождает отдельный слой без уничтожения всего каскада.
- Bottom sheet и carousel проецируют default decay прямо в точку покоя, без
  построения неиспользуемой sampling-модели; guards и IEEE-порядок сохраняются.
- `createStateCascade` в `./behaviors`: property-level каскад визуальных намерений; верхний активный слой владеет только своими ключами, скрытые изменения не эмитят работу, а снятие press/drag/exit раскрывает актуальный нижний target.
