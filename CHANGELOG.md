...
- `createMotionBinding` связывает модель компонента с именованными визуальными
  ролями без отдельного store. Неизменённые цели не перезапускают анимации;
  повторный ввод и очистка обслуживаются одной привязкой. Рецепт не зависит от DOM. Практические примеры покрывают загрузку и независимые
  индикаторы выбранной страницы/клавиатурного фокуса.

### Изменено
- `./animate`: `createAnimateScope(root)` связывает локальные селекторы с одной
  уборкой компонента. Завершённые вызовы не накапливаются, `destroy()` отменяет
  оставшиеся и обезвреживает поздние обработчики. Добавлены исполняемые примеры
  для DOM, React StrictMode и Solid; очистка сохраняет позу, а не делает revert.
- `MotionValue.setTarget` сохраняет прошедшее время при частом перенаправлении:
  слежение за указателем больше не застывает при новой цели на каждом кадре.
  Повтор активной цели не перезапускает траекторию; явный перезапуск — через `stop()`.
...
- `springAsEasing` больше не обрывается на конце. Прежняя проекция масштабировала
  ...
  и ненулевая скорость 0.212. Теперь шкала — горизонт допуска, а хвост добирается
  эрмитовой C¹-запечаткой; отклонение от настоящей пружины не превышает
  `CONVERGENCE_THRESHOLD`. Форма кривой изменилась: для ζ=1 горизонт вырос с 4.605
  до 8.262 в единицах ω₀t, поэтому в тот же отрезок укладывается больше траектории.
- Сэмплирование перестало валидировать параметры на каждом вызове: валидация
  выполняется один раз при построении.

### Добавлено
- `createStateCascade` в `./behaviors` разрешает визуальные намерения по свойствам;
  перекрытые изменения не уведомляют исполнителя. `batch` объединяет смену
  нескольких состояний в одну цель без промежуточных перезапусков.
- Каскад сохраняет порядок вложенных уведомлений и доставку при ошибках
  подписчиков. `destroy()` прекращает доставку, `layer.destroy()` освобождает
  отдельный слой; устаревшее чтение входа не перезаписывает новое состояние.
- `createBottomSheet` и `createCarousel` вычисляют точку покоя инерции без построения
  неиспользуемой модели движения. Проверки входа и порядок IEEE-операций сохранены.
...
- `compileSpringPlan` принимает `maxValueError`: абсолютный
  бюджет ошибки в единицах numeric `from/to` переводится в нормализованный допуск
  до кэша и сегментера; более строгий из `tolerance` и бюджета результата побеждает.
- Код `LM172`: `maxValueError` не является положительным конечным числом.
