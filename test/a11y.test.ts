...
/** Управляемый системный matchMedia-стаб (с интроспекцией слушателей). */
...
describe('a11y/policy: режимы', () => {
  ...
  // Пин гардов невалидного режима (нота ревью PR #29).
  it("set() с мусором — no-op: mode и эффект не меняются", () => {
    const cfg = createMotionConfig({ reducedMotion: 'always' });
    cfg.set('garbage' as never);
    expect(cfg.mode).toBe('always');
    expect(cfg.prefersReduced()).toBe(true);
  });

  it("мусорный reducedMotion в конструкторе → 'system'", () => {
    const cfg = createMotionConfig({ reducedMotion: 'nope' as never });
    expect(cfg.mode).toBe('system');
  });

  it('destroy(): отпускает системного слушателя и глушит уведомления', () => {
...
describe('a11y/policy: matchMedia-шов для остальных субпутей', () => {
  ...
  // Всё семейство prefers-reduced-motion синтезируется из политики —
  // никаких противоречий «reduce=true И no-preference=true» (нота PR #29).
  it('no-preference — тоже из ПОЛИТИКИ: при always → false', () => {
    const cfg = createMotionConfig({ reducedMotion: 'always', matchMedia: systemMedia(false).mm });
    expect(cfg.matchMedia('(prefers-reduced-motion: no-preference)').matches).toBe(false);
    const cfg2 = createMotionConfig({ reducedMotion: 'never', matchMedia: systemMedia(true).mm });
    expect(cfg2.matchMedia('(prefers-reduced-motion: no-preference)').matches).toBe(true);
  });

  it('голая форма (prefers-reduced-motion) — булев контекст = reduce', () => {
    const cfg = createMotionConfig({ reducedMotion: 'always', matchMedia: systemMedia(false).mm });
    expect(cfg.matchMedia('(prefers-reduced-motion)').matches).toBe(true);
  });
});
...
describe('a11y-api-surface-pin', () => {
  ...
  it('форма конфига (исчерпывающе)', () => {
    ...
    expect(Object.keys(cfg).sort()).toEqual(['destroy', 'matchMedia', 'mode', 'onChange', 'prefersReduced', 'set']);
  });
