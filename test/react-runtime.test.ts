...
describe('useVelocity / useTransform — render-value деривации (#104)', () => {
  it('useTransform: отражает fn(value) и обновляется по кадрам', () => {
    // Диверсия «subscribe = no-op» → out застревает на fn(0) после setTarget → RED.
    const clock = makeClock();
    let mv!: ReturnType<typeof useMotionValue>;
    function Probe(): ReturnType<typeof createElement> {
      mv = useMotionValue(0, SPRING, clock.requestFrame);
      const doubled = useTransform(mv, (v) => v * 2);
      return createElement('div', { id: 'p' }, doubled.toFixed(2));
    }
    const c = mount(createElement(Probe));
    const txt = (): number => Number((c.querySelector('#p') as HTMLElement).textContent);
    expect(txt()).toBe(0); // fn(0) = 0
    act(() => mv.setTarget(50));
    act(() => clock.drain());
  ...
  it('useVelocity: >0 в полёте, ~0 в покое', () => {
    ...
    function Probe(): ReturnType<typeof createElement> {
      mv = useMotionValue(0, SPRING, clock.requestFrame);
      const v = useVelocity(mv);
      return createElement('div', { id: 'p' }, v.toFixed(4));
    }
    const c = mount(createElement(Probe));
    const txt = (): number => Number((c.querySelector('#p') as HTMLElement).textContent);
    expect(txt()).toBe(0); // старт в покое
    act(() => {
      mv.setTarget(100);
      clock.drain(2); // пара кадров — пружина разгоняется
    });
    expect(txt()).toBeGreaterThan(0); // реально летит
    act(() => clock.drain()); // до оседания
    expect(txt()).toBeLessThan(0.01); // покой → скорость угасла
  });

  it('StrictMode: подписки-деривации без краша, корректно трекают живой MotionValue', () => {
    // Изолируем ИМЕННО slice-3 хуки: MotionValue создан тестом (жив весь прогон),
    // а не useMotionValue — чтобы StrictMode гонял только subscribe→cleanup→subscribe
    // наших useVelocity/useTransform (они не владеют MV, подписка чистая). Диверсия
    // «не отписываться в cleanup» → двойная подписка под StrictMode.
    const clock = makeClock();
    const mv = createBoundValue({ initial: 0, spring: SPRING, requestFrame: clock.requestFrame });
    function Probe(): ReturnType<typeof createElement> {
      const v = useVelocity(mv);
      const t = useTransform(mv, (x) => x + 1);
      return createElement('div', { id: 'p' }, `${v.toFixed(2)}|${t.toFixed(2)}`);
    }
    let c!: HTMLElement;
    expect(() => {
      c = mount(createElement(StrictMode, null, createElement(Probe)));
    }).not.toThrow();
    act(() => {
      mv.setTarget(10);
      clock.drain();
    });
    const [, tStr] = (c.querySelector('#p') as HTMLElement).textContent!.split('|');
    expect(Number(tStr)).toBeCloseTo(11, 1); // fn(10)=11 после оседания
    mv.destroy();
  });
});
