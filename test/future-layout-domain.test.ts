...
describe('сопряжённая траектория: serialized P → reciprocal Q → monotonic A', () => {
  ...
  it('RED п.9: crossfade A(t) монотонна — old snapshot не появляется повторно', () => {
    ...
    const blend = explicitLinearValues(artifact.blendEasing);
...
/** Тестовый parser намеренно независим от diagnostics receipt. */
function explicitLinearValues(css: string): number[] {
  if (!css.startsWith('linear(') || !css.endsWith(')')) throw new TypeError('surface easing is not linear()');
  return css.slice(7, -1).split(',').map((part) => {
    const tokens = part.trim().split(/\s+/);
    if (tokens.length !== 2 || !tokens[1]!.endsWith('%')) throw new TypeError('surface stop is not explicit');
