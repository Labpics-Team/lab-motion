 ...
 * НЕ-ЦЕЛЬ среза, отложено до #127 (ленивый dpdt освободит байты). Оба сценария
 * стоят вплотную к регрессионным потолкам: любой рост фасада ломает гейт, а геттер velocity
...
describe('пункт Б (#93): хендл animate() — агрегированные контролы БЕЗ velocity (отложено до #127)', () => {
  ...
  it('публичного velocity на хендле НЕТ — решение среза 6 отложено до #127', () => {
