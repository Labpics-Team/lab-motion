...
describe('C8-10 interpolate: progress-клампинг + путь hsl/rgb (156,158,160)', () => {
  const black = pc('#000');
  const white = pc('#fff');
  it('progress: t=0→from, t=1→to, t=0.5→линейно-световая середина 180', () => {
    expect(interpolateColor(black, white, 0)).toBe('rgb(0, 0, 0)');
    expect(interpolateColor(black, white, 1)).toBe('rgb(255, 255, 255)');
    // default 'linear' (2026-07-03): √(255²·0.5) = 180.31 → 180 (не 128!).
    // Кусает и sqrt-мутанты (замена на lerp вернула бы 128).
    expect(interpolateColor(black, white, 0.5)).toBe('rgb(180, 180, 180)');
  });
  it("легаси {space:'srgb'}: t=0.5 → гамма-середина 128 (CSS Color 4 §13.1)", () => {
    expect(interpolateColor(black, white, 0.5, { space: 'srgb' })).toBe('rgb(128, 128, 128)');
    // Мутант `options?.space !== 'srgb'` → `=== 'srgb'` инвертирует оба пина.
  });
...
describe('C11-13 interpolate: каналы/alpha/hue-wraparound (179-213)', () => {
  it('alpha-интерполяция: rgba(0,0,0,0)→rgba(255,255,255,1)@.5 = rgba(180, 180, 180, 0.5)', () => {
    // EXACT-string кусает: channel-арифметику, Math.round, ветку a>=1, +a.toFixed.
    // Каналы — линейно-световая середина 180; alpha — ЛИНЕЙНАЯ 0.5 (покрытие,
    // не свет): sqrt-мутант на альфе дал бы 0.7071 → пин кусает.
    const c0 = pc('rgba(0, 0, 0, 0)');
    const c1 = pc('rgba(255, 255, 255, 1)');
    expect(interpolateColor(c0, c1, 0.5)).toBe('rgba(180, 180, 180, 0.5)');
  });
...
describe('C16 mixColor: невалид-фоллбек t<0.5?from:to (строка 173)', () => {
  ...
  it('оба валидны: смешивает (не фоллбек) — linear-light 180', () => {
    expect(mixColor('rgb(0, 0, 0)', 'rgb(255, 255, 255)', 0.5)).toBe('rgb(180, 180, 180)');
  });
});
...
describe('C19 interpolate путь: rgb+hsl и hsl+rgb → rgb-путь (строка 160)', () => {
  it('rgb+hsl → rgb-путь: #ff0000→hsl(120,100%,50%)@.5 = rgb(180, 180, 0)', () => {
    // Один не-hsl → RGB-путь (linear-light: √(255²·0.5)=180). Мутант, берущий
    // hsl-путь при from.format≠hsl, дошёл бы до interpolateHsl(rgb-from) →
    // from.hsl undefined → бросок. Оракул rgb(...) кусает.
    expect(interpolateColor(pc('#ff0000'), pc('hsl(120, 100%, 50%)'), 0.5)).toBe('rgb(180, 180, 0)');
  });
  it('hsl+rgb → rgb-путь: hsl(120,100%,50%)→#ff0000@.5 = rgb(180, 180, 0)', () => {
    expect(interpolateColor(pc('hsl(120, 100%, 50%)'), pc('#ff0000'), 0.5)).toBe('rgb(180, 180, 0)');
  });
});
