 ...
 * Спека: §2.1.4 (стражи и деградации), §2.1.5 (коды ошибок), §7.3, P1
 ...
 *     красный (смысловой код пинится буквально, §2.1.5).
 */
...
describe('projection: валидация createProjector — коды MotionParamError', () => {
  ...
  it('пустой id', () => {
    ...
    expect(() => createProjector([{ id: '', first: OK, last: OK }])).toThrow('LM079');
  });
  ...
  it('дубль id', () => {
    ...
    expect(() => createProjector(nodes)).toThrow('LM080');
  });
  ...
  it('неизвестный parent', () => {
    ...
    expect(() => createProjector(nodes)).toThrow(MotionParamError);
    expect(() => createProjector(nodes)).toThrow('LM082');
  });

  it('цикл: самоссылка', () => {
    const nodes = [{ id: 'a', parent: 'a', first: OK, last: OK }];
    expect(() => createProjector(nodes)).toThrow(MotionParamError);
    expect(() => createProjector(nodes)).toThrow('LM083');
  });

  it('цикл из двух узлов: бросок с кодом цикла LM083', () => {
    const nodes = [
