 ...
 * «createDomProjection is not a function»: RED for the right reason.
 *
 * Mutation proof:
 ...
 *   - Слать драйверу cap.first вместо first: undefined для узлов активного
 *     полёта → «отложенный play: C⁰» красный (телепорт на снимок capture-времени).
 *   - Не потреблять captured в play()/cancel() → «повторный play без capture
 *     бросает» красный (телепорт по протухшему снимку + retention).
 *   - Вернуть подъём без Set посещённых (ранний return на первом замеренном) →
 *     «враждебный parent-цикл» красный (createProjector бросил бы parent cycle).
 */
...
describe('projection/dom: capture потребляется ОДНОКРАТНО (§2.4)', () => {
  it('повторный play() без нового capture — ранний MotionParamError, до любых записей/замеров; полёт не тронут', () => {
    const world = makeWorld();
    const clock = makeClock();
    const A = world.el('A', { x: 0, y: 0, width: 100, height: 100 });
    const dom = createDomProjection(domOptions(world, clock));
    dom.capture([A]);
    A.rect = { x: 300, y: 0, width: 100, height: 100 };
    dom.play();
    ...
    clock.drain(16);
    expect(dom.playing).toBe(false); // и честно доезжает
  });

  it('cancel() тоже потребляет снимок: capture → cancel → play бросает', () => {
    const world = makeWorld();
    const clock = makeClock();
    const A = world.el('A', { x: 0, y: 0, width: 100, height: 100 });
    const dom = createDomProjection(domOptions(world, clock));
  ...
  it('перехват жив: capture → play → capture(mid-flight) → play непрерывен и не бросает', () => {
    const world = makeWorld();
    const clock = makeClock();
    const A = world.el('A', { x: 0, y: 0, width: 100, height: 100 });
    const dom = createDomProjection(domOptions(world, clock));
    ...
    dom.play();
    clock.step(16);
    dom.capture([A]); // новый снимок mid-flight — потребление не ломает перехват
    A.rect = { x: 500, y: 0, width: 100, height: 100 };
    expect(() => dom.play()).not.toThrow();
    expect(dom.playing).toBe(true);
  });
});

describe('projection/dom: visual pickup на момент play (first: undefined → драйвер)', () => {
  it('отложенный play после capture mid-flight: C⁰ по последнему КАДРУ, не по снимку capture-времени', () => {
    const world = makeWorld();
    const clock = makeClock();
    const A = world.el('A', { x: 0, y: 0, width: 100, height: 100 });
    const dom = createDomProjection(domOptions(world, clock));
    dom.capture([A]);
    A.rect = { x: 1000, y: 0, width: 100, height: 100 };
    dom.play();
    clock.step(16);
    clock.step(16); // p̂₁ на момент capture

    dom.capture([A]); // аналитический снимок на p̂₁
    clock.step(16);
    clock.step(16);
    clock.step(16); // пружина уехала: p̂₂ > p̂₁ (отложенный play)

    const before = flightTransformWrites(world.writes(A));
    const lastT = parseTranslateScale(before[before.length - 1].value!)!;
    const visualBefore = 1000 + lastT.tx; // визуальный x последнего кадра (p̂₂)
    expect(visualBefore).toBeGreaterThan(0);

    A.rect = { x: 500, y: 0, width: 100, height: 100 }; // новая цель
    dom.play();

    const after = flightTransformWrites(world.writes(A));
    expect(after.length).toBe(before.length + 1); // синхронный первый кадр нового полёта
    const firstNew = parseTranslateScale(after[after.length - 1].value!)!;
    const visualAfter = 500 + firstNew.tx;
    // Мутация «слать cap.first» → визуальный скачок к V(p̂₁) — телепорт → RED.
    expect(visualAfter).toBeCloseTo(visualBefore, 9);
  });
});

describe('projection/dom: враждебный parent-цикл — никогда не бросаем (тотальность к состоянию DOM)', () => {
  it('A.parentElement=B, B.parentElement=A: play() не бросает, оба узла — корни, инлайны чистые после cancel', () => {
    const world = makeWorld();
    const clock = makeClock();
    const A = world.el('A', { x: 0, y: 0, width: 100, height: 100 });
    const B = world.el('B', { x: 10, y: 10, width: 50, height: 50 });
    A.parentElement = B; // враждебный 2-цикл: DOM так не умеет, фейк — умеет
    B.parentElement = A;
    const dom = createDomProjection(domOptions(world, clock));

    dom.capture([A, B]);
    A.rect = { x: 200, y: 0, width: 100, height: 100 };
    B.rect = { x: 40, y: 10, width: 50, height: 50 };
    expect(() => dom.play()).not.toThrow(); // цикл → корни, НЕ parent-cycle-бросок
    expect(dom.playing).toBe(true);

    // Оба — корни: первый кадр каждого = чистый root-flip (без counter-коррекции).
    const a0 = parseTranslateScale(flightTransformWrites(world.writes(A))[0].value!)!;
    expect(a0.tx).toBeCloseTo(-200, 9);
    expect(a0.ty).toBeCloseTo(0, 9);
    expect(a0.sx).toBeCloseTo(1, 9);
    const b0 = parseTranslateScale(flightTransformWrites(world.writes(B))[0].value!)!;
    expect(b0.tx).toBeCloseTo(-30, 9); // 10 − 40; «ребёнок под A» дал бы +170
    expect(b0.sx).toBeCloseTo(1, 9);
