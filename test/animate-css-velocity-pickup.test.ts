 ...
 * (канон projectCssV0 — доминанта всегда по целевому диапазону):
...
describe('animate css-канал: подхват скорости при перехвате (Класс Б, пин #93 срез 4)', () => {
  // @todo-css-module: скоростная проекция css (projectCssV0) не входит в базу и live
  // (директива R3a: v0 прогресс-полосы css всегда 0) — уезжает в композируемый
  // css-модуль вместе с проекцией. До R3c-1 пин был зелёным по артефакту:
  // css-снимок live-v1 отдавал {p:1,ṗ:0}, ребейз к цели читался оракулом как
  // огромная «унаследованная скорость». Точный снимок R3c-1 обнажил v=0.
  it.skip('bite: перехват на середине наследует |v| > 0.65·|v_analytic| (RED до среза: v=0)', () => {
    const f = fakeEl({ width: '0px' });
    const clock = makeClock();
  ...
  // @todo-css-module: плато требует унаследованного css-импульса — проекция
  // v0 живёт в будущем композируемом css-модуле (директива R3a: css v0 = 0).
  it.skip('обратный ретаргет: импульс держит границу (плато у захвата), затем движение к цели', () => {
...
describe('animate css-канал: проекция скорости точна (Класс А)', () => {
  // @todo-css-module: точная css-проекция скорости (projectCssV0) — вне базы и
  // live (директива R3a); формулы сверки сохранены для будущего css-модуля.
  it.skip('spring→spring: унаследованная скорость — ровно ṗ̂·range старого рана', () => {
    const r = runCssPickup({ mode1: { spring: SPRING }, seekMs: 80, to2: '300px' });
  ...
  // @todo-css-module: точная css-проекция скорости (projectCssV0) — вне базы и
  // live (директива R3a); формулы сверки сохранены для будущего css-модуля.
  it.skip('tween→spring: линейный ease, k=0.5 → ровно range/duration = 250 units/s', () => {
    const r = runCssPickup({
  ...
  // @todo-css-module: точная css-проекция скорости (projectCssV0) — вне базы и
  // live (директива R3a); формулы сверки сохранены для будущего css-модуля.
  it.skip('цвет (коллинеарный ретаргет): наследует |v| > 0.6·|v_analytic| доминантного канала', () => {
  ...
  it('неколлинеарный ретаргет: доминанта по НОВОМУ спану — без взрывного усиления', () => {
    ...
    // кадр и висел на клампе («violent flash»). Канон projectCssV0 —
    // доминанта всегда по ЦЕЛЕВОМУ диапазону.
...
describe('animate css-канал: скорость против аналитической (Класс В, seeded fuzz)', () => {
  // @todo-css-module: наследование css-скорости требует projectCssV0 (вне базы
  // и live, директива R3a). До R3c-1 фазз был зелёным вакуумно: no-op seek
  // пружины оставлял wMid=undefined, NaN проваливал err-гард без броска.
  // Точный seek R3c-1 сделал оракул честным — и он честно видит v=0.
  it.skip('300 прогонов: |v_inherited − v_analytic| ≤ 1% шкалы; записи конечны', () => {
