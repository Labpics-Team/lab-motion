...
describe('animate css-канал: подхват скорости при перехвате (Класс Б, пин #93 срез 4)', () => {
  ...
  // @todo-R3c: pickup-parity: траектории cssAt/projectCssV0 старых лейнов; новая политика css (шов formatCssAt, C0-рестарт) — R3a; точные пины пере-выводятся в R3c
  it.skip('C⁰: первая запись нового рана — ровно захваченное значение (ребейз, не p=0 старого спана)', () => {
  ...
  // @todo-R3c: pickup-parity: траектории cssAt/projectCssV0 старых лейнов; новая политика css (шов formatCssAt, C0-рестарт) — R3a; точные пины пере-выводятся в R3c
  it.skip('явная пара [from, to] отключает подхват: старт из покоя с явного from', () => {
