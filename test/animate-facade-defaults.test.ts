 ...
 *   { ease } без duration → duration.normal (мс).
 * Смена дефолта (например spring.default → gentle) обязана сделать файл красным.
 *
 * Метод: траектория сверяется bit-exact с публичной аналитикой ядра
 * (readCompositorSpring / easing.fn) на детерминированных шаг-часах —
 * не «похоже», а тождественно.
 *
 * ── RED PROOF ────────────────────────────────────────────────────────────────
 * Рождён на пустой заглушке — все тесты красные.
 */
...
describe('./animate — дефолты из ./tokens (Класс Б, характеризация)', () => {
  it('без опций: траектория тождественна spring.default (bit-exact)', () => {
    const f = fakeEl();
  ...
  it('{ duration } без ease: изинг — easing.standard.fn (bit-exact в середине)', () => {
    const f = fakeEl();
    const clock = makeClock();
  ...
  it('{ ease } без duration: длительность — duration.normal из токенов', async () => {
    const f = fakeEl();
    const clock = makeClock();
    ...
    clock.step(duration.normal - 1); // elapsed = normal - 1 → ещё в полёте
    expect(translateXSeries(f.writes).at(-1)!).toBeLessThan(100);
    clock.step(1); // elapsed = duration.normal → осел ровно на цели
