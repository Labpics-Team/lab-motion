...
describe('svg-morph: составные пути морфятся пер-подконтурно', () => {
  ...
  it.each([
    // [команда, rel-форма первого подконтура, конец первого подконтура]
    ['L', 'M 10 10 L 20 20', [20, 20]],
    ['l', 'M 10 10 l 10 10', [20, 20]],
    ['H', 'M 10 10 H 30', [30, 10]],
    ['h', 'M 10 10 h 20', [30, 10]],
    ['V', 'M 10 10 V 30', [10, 30]],
    ['v', 'M 10 10 v 20', [10, 30]],
    ['C', 'M 10 10 C 15 10 20 15 20 20', [20, 20]],
    ['c', 'M 10 10 c 5 0 10 5 10 10', [20, 20]],
    ['S', 'M 10 10 S 20 15 20 20', [20, 20]],
    ['Q', 'M 10 10 Q 15 10 20 20', [20, 20]],
    ['T', 'M 10 10 T 20 20', [20, 20]],
    ['A', 'M 10 10 A 5 5 0 0 1 20 20', [20, 20]],
  ] as Array<[string, string, [number, number]]>)(
    'трекер точки: конец команды %s — корректная база относительного m (rel ≡ abs)',
    (_cmd, head, [ex, ey]) => {
      // Класс (найден выжившими мутантами QA #47): трекер splitSubpaths был
      // mutation-слаб по V/H/C/S/Q/T/A — относительный m после этих команд
      // не встречался в тестах. Дифференциал: относительная и абсолютная
      // записи одного пути обязаны давать байт-идентичный морф.
      const relForm = `${head} m 5 5 L 40 40`;
      const absForm = `${head} M ${ex + 5} ${ey + 5} L 40 40`;
      const target = 'M 0 0 L 20 0 L 20 20 Z M 30 30 L 50 30 L 50 50 Z';
      for (const p of [0.25, 0.5, 0.75]) {
        expect(interpolatePath(relForm, target, { samples: 8 })(p)).toBe(
          interpolatePath(absForm, target, { samples: 8 })(p),
        );
      }
    },
  );
