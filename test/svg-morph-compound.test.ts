...
describe('svg-morph: составные пути морфятся пер-подконтурно', () => {
  ...
  it('относительный m в середине пути абсолютизируется корректно', () => {
    // Тот же «O», но второй подконтур задан относительным m от конца первого.
    // После Z текущая точка = старт подконтура (0,0) → m 5 5 ≡ M 5 5.
    const relForm = 'M 0 0 L 20 0 L 20 20 L 0 20 Z m 5 5 L 15 5 L 15 15 L 5 15 Z';
    const f = interpolatePath(relForm, O_TO, { samples: 16 });
    const mid = f(0.5);
    expect(countM(mid)).toBe(2);
    // Первая точка второго подконтура на p=0.5 — среднее (5,5)↔(18,5) = (11.5, 5).
    const cmds = parsePath(mid);
    const secondM = cmds.filter((c) => c.type.toUpperCase() === 'M')[1]!;
    expect(secondM.values[0]).toBeCloseTo(11.5, 3);
    expect(secondM.values[1]).toBeCloseTo(5, 3);
  });
