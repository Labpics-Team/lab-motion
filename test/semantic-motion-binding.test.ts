...
it('предел общего снимка проверяется до чтения значений переполненной роли', () => {
  ...
  expect(() => view.update(undefined)).toThrowError(expect.objectContaining({ code: 'LM180' }));
...
it.each([null, undefined, 1, 'project', {}])('LM179: некорректная проекция %s не читает порты', project => {
  const read = vi.fn();
  const targets = { get panel() { read(); return () => {}; } };
  expect(() => create(project as never, targets)).toThrow(expect.objectContaining({ code: 'LM179' }));
  expect(read).not.toHaveBeenCalled();
});

it.each([null, undefined, [], {}, { panel: 1 }])('LM179: некорректная карта портов %s', targets => {
  const project = vi.fn(() => ({ panel: { x: 1 } }));
  expect(() => create(project, targets as never)).toThrow(expect.objectContaining({ code: 'LM179' }));
  expect(project).not.toHaveBeenCalled();
});

it('LM179: переполненная карта портов отвергается до чтения функций', () => {
  ...
  expect(() => create(() => ({ panel: { x: 1 } }), targets as never)).toThrow(expect.objectContaining({ code: 'LM179' }));
  expect(read).not.toHaveBeenCalled();
});

it.each([null, 3, 'handle', {}, { cancel: 1 }, Promise.resolve()])('LM179: неверный ресурс %s не оставляет старых владельцев', result => {
  ...
  view.update(1);
  expect(() => view.update(2)).toThrow(expect.objectContaining({ code: 'LM179' }));
  expect(view.state).toBe('failed');
  for (const h of [a, b, next]) expect(h.cancel).toHaveBeenCalledOnce();
  view.destroy();
});

it('порт без ресурса и callable handle остаются разрешёнными соседями LM179', () => {
