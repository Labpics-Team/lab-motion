...
animate(el, ${objectLiteral(props)}${options ? `, ${objectLiteral(options)}` : ''});
`;
  ...
  it('stagger по нескольким целям: delay каждой цели совпадает с фасадом', async () => {
    ...
    const source = `import { animate } from '@labpics/motion/animate';
animate(els, { opacity: 1 }, { stagger: 50 });
`;
    const p = await plan(source);
    const literal = p!.edits[1]!.replacement.replace(/^, /, '').replace(/\)$/, '');
...
describe('#240: грамматика понижения фасада', () => {
  it('понижение автоматическое: выбирать на каждый вызов автор не должен', async () => {
...
`);
  ...
  it('@motion-runtime оставляет вызов фасаду — без отказа (осознанный выбор)', async () => {
    const p = await plan(`import { animate } from '@labpics/motion/animate';
/* @motion-runtime */ animate(el, { x: 240 });
`);
...
`);
  ...
  const REFUSED: [name: string, call: string][] = [
    ['tween duration', `animate(el, { x: 1 }, { duration: 300 });`],
    ['ease', `animate(el, { x: 1 }, { ease: (t) => t });`],
    ['CSS-канал', `animate(el, { '--x': ['0px', '1px'] });`],
    ['цвет строкой', `animate(el, { backgroundColor: 'red' });`],
    ['трек из трёх', `animate(el, { x: [0, 10, 0] });`],
    ['props переменной', `animate(el, props);`],
    ['spread в props', `animate(el, { ...base });`],
    ['дубликат ключа', `animate(el, { x: 1, x: 2 });`],
    ['пустые props', `animate(el, {});`],
    ['неизвестный options-ключ', `animate(el, { x: 1 }, { onComplete: cb });`],
    ['spring переменной', `animate(el, { x: 1 }, { spring: s });`],
    ['optional call', `animate?.(el, { x: 1 });`],
    ['комментарий в тривиа-зоне', `animate(/* h */ el, { x: 1 });`],
  ];
...
`);
  ...
  it('использованный результат — отказ: понижённый вызов не отдаёт контролы', async () => {
    const p = await plan(`import { animate } from '@labpics/motion/animate';
const c = animate(el, { x: 1 });
`);
    expect(p!.edits).toHaveLength(0);
    expect(p!.refusals[0]!.reason).toContain('не публикует owner');
  });

  it('непонижаемый вызов НЕ роняет сборку — он просто остаётся фасадом', async () => {
    const out = await transform(`import { animate } from '@labpics/motion/animate';
animate(el, { x: 1 }, { duration: 300 });
`);
    expect(out).toBeUndefined();
  });

  it('strict требует полного стирания и называет причину', async () => {
    const plugin = motionCompiler({ strict: true });
    const code = `import { animate } from '@labpics/motion/animate';
animate(el, { x: 1 }, { duration: 300 });
`;
    const ast = await parseAstAsync(code);
    expect(() => plugin.transform.call({ parse: () => ast }, code, '/app/m.ts'))
      .toThrow(/непониженный фасадный вызов .*tween/);
  });
...
animate(el, { x: 1 }, { spring: { mass: 1, stiffness: 0, damping: 26 } });
...
function f(animate) { animate(el, { x: 1 }); }
`)).toBeUndefined();
  });
});
...
describe('#240: артефакт понижения', () => {
  ...
  it('grammar: from transform-каналов — identity, opacity без пары — 1', async () => {
    const p = await plan(`import { animate } from '@labpics/motion/animate';
animate(el, { scale: 1.5, opacity: 0.2, rotate: 45 });
`);
...
describe('#240: квитанция объясняет, почему фасад ещё в бандле', () => {
  async function budgetOf(code: string) {
    let report: { lowered: number; facadeCallsKept: number } | undefined;
  ...
  it('всё доказуемое понижено, остальное посчитано с причиной', async () => {
...
export function c(el, v) { animate(el, { x: v }); }
export function d(el) { animate(el, { x: 1 }, { duration: 300 }); }
`);
    expect(report.lowered).toBe(2);
    expect(report.facadeCallsKept).toBe(2);
  });

  it('модуль без остатка: фасад можно вытряхивать целиком', async () => {
    const report = await budgetOf(`import { animate } from '@labpics/motion/animate';
export function a(el) { animate(el, { x: 240 }); }
export function b(el) { animate(el, { opacity: 1 }); }
`);
    expect(report.lowered).toBe(2);
    expect(report.facadeCallsKept).toBe(0);
  });

  it('buildStart сбрасывает счётчики (watch-ребилд не наследует)', async () => {
    const source = `import { animate } from '@labpics/motion/animate';
export function a(el, v) { animate(el, { x: v }); }
`;
    let report: { facadeCallsKept: number } | undefined;
    const plugin = motionCompiler({ onBudget: (r) => { report = r; } });
    ...
    for (let build = 0; build < 3; build++) {
      ...
      plugin.buildEnd();
      expect(report!.facadeCallsKept, `сборка ${build}`).toBe(1);
    }
  });
});

/** Цель с ЧИТАЕМЫМ временем анимации — иначе состояние прогона неизвестно. */
function timedEl(initialStyle: Record<string, string> = {}) {
  const inline = new Map(Object.entries(initialStyle));
  const animateCalls: { keyframes: unknown; timing: Record<string, unknown> }[] = [];
  const animations: { currentTime: number; playState: string }[] = [];
  const el = {
    style: {
      setProperty: (n: string, v: string) => { inline.set(n, v); },
      getPropertyValue: (n: string) => inline.get(n) ?? '',
    },
    animate(keyframes: unknown, timing: Record<string, unknown>) {
      animateCalls.push({ keyframes, timing });
      const animation = { currentTime: 0, playState: 'running', cancel(): void {} };
      animations.push(animation);
      return animation;
    },
  };
  return { el, animateCalls, animations };
}

describe('#240: понижённый вызов продолжает с текущего состояния', () => {
  const runTo = (el: unknown, to: number): void => {
    animateFacadeCompiled(
      el as never,
      artifactOf(facadeArtifactLiteral([['transform', [['x', 0, to]]]])),
    );
  };

  it('после завершения второй вызов стартует с КОНЦА первого (нет прыжка назад)', () => {
    const target = timedEl();
    runTo(target.el, 240);
    target.animations[0]!.currentTime = artifactOf(
      facadeArtifactLiteral([['transform', [['x', 0, 240]]]]),
    ).d;
    target.animations[0]!.playState = 'finished';
    runTo(target.el, 0);
    const second = target.animateCalls[1]!.keyframes as Record<string, unknown>[];
    expect(second[0]).toEqual({ offset: 0, transform: 'translateX(240px)' });
    expect(second[1]).toEqual({ offset: 1, transform: 'none' });
  });

  it('прерывание на лету стартует из ТЕКУЩЕЙ позиции (не с конца и не с нуля)', () => {
    const target = timedEl();
    const artifact = artifactOf(facadeArtifactLiteral([['transform', [['x', 0, 240]]]]));
    runTo(target.el, 240);
    target.animations[0]!.currentTime = artifact.d / 2;
    runTo(target.el, 0);
    const start = (target.animateCalls[1]!.keyframes as Record<string, unknown>[])[0]!;
    const x = Number(/translateX\(([-\d.]+)px\)/.exec(String(start['transform']))![1]);
    // Пружина на середине времени уже далеко за половиной пути, но НЕ в конце.
    expect(x).toBeGreaterThan(120);
    expect(x).toBeLessThan(240);
  });

  it('отменённый прогон следа не оставляет (playState idle)', () => {
    const target = timedEl();
    runTo(target.el, 240);
    target.animations[0]!.playState = 'idle';
    runTo(target.el, 0);
    const start = (target.animateCalls[1]!.keyframes as Record<string, unknown>[])[0]!;
    expect(start).toEqual({ offset: 0, transform: 'none' });
  });

  it('нечитаемое время — состояние неизвестно, действует каскад фасада', () => {
    const target = fakeEl({}, true);
    runTo(target.el, 240);
    runTo(target.el, 0);
    const start = (target.animateCalls[1]!.keyframes as Record<string, unknown>[])[0]!;
  ...
  it('не-transform группа стартует из живого стиля (opacity из CSS)', () => {
    const target = fakeEl({ opacity: '0.25' }, true);
    animateFacadeCompiled(
      target.el as never,
      artifactOf(facadeArtifactLiteral([['opacity', [['opacity', 1, 1]]]])),
    );
    const frames = target.animateCalls[0]!.keyframes as Record<string, unknown>[];
    expect(frames[0]).toEqual({ offset: 0, opacity: 0.25 });
    expect(frames[1]).toEqual({ offset: 1, opacity: 1 });
  });

  it('разные элементы не делят состояние (реестр по элементу)', () => {
    const a = timedEl();
    const b = timedEl();
    runTo(a.el, 240);
    a.animations[0]!.playState = 'finished';
    a.animations[0]!.currentTime = 1e9;
    runTo(b.el, 240);
    const frameB = (b.animateCalls[0]!.keyframes as Record<string, unknown>[])[0];
    expect(frameB).toEqual({ offset: 0, transform: 'none' });
  });
});
