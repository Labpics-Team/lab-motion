...
describe('animate группа x/y/scale: единый clock (Класс Б, пин #93 срез 4)', () => {
  ...
  it.skip('группа из 3 каналов подписывает РОВНО ОДИН rAF-колбэк на кадр', () => {
    const f = fakeEl();
  ...
  // @todo-R3c: main-lane: единый rAF-clock старых лейнов; live-v1 — MotionValue per-lane (когерентность группы — R3c)
  it.skip('spring: каждый кадр — атомарный вектор с одного t (пропорция диапазонов)', () => {
    const f = fakeEl();
    const clock = makeClock();
...
describe('animate группа: перехват снимает когерентный вектор (Класс Б)', () => {
  ...
  // @todo-R3c: main-lane: единый rAF-clock старых лейнов; live-v1 — MotionValue per-lane (когерентность группы — R3c)
  it.skip('после перехвата остаётся один frame-loop (кадры прежнего юнита инертны)', () => {
    const f = fakeEl();
    const clock = makeClock();
    let subs = 0;
    const rf = (cb: (ts?: number) => void): number => {
