...
describe('keyframes валидация: валидное ПРИНЯТО + точный code-only контракт', () => {
  it('невызываемый scalar easing отвергается стабильным LM163', () => {
    expect(() => keyframes({
      values: [0, 1],
      easing: 42 as never,
      requestFrame: () => 1,
...
describe('sampleKeyframes границы: p=NaN/±Infinity, точные стыки, нулевой сегмент', () => {
  ...
  it('p=NaN → canonical progress 0 → values[0]', () => {
...
describe('keyframes computeAt: направление loop/reverse/mirror по циклам', () => {
  ...
  it('mirror keeps forward easing over reversed values, unlike reverse time', () => {
    const quadratic = (t: number): number => t * t;
    const asymmetric = { values: [0, 100, 20], duration: 1, repeat: 2, easing: quadratic } as const;
    expect(probeAt({ ...asymmetric, repeatType: 'reverse' }, 1.25)).toBe(80);
    expect(probeAt({ ...asymmetric, repeatType: 'mirror' }, 1.25)).toBe(40);
  });
