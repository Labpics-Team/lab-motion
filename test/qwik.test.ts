...
describe('qwik: useSpring — управление через сигнал-цель', () => {
  ...
  it('full→reduce инвалидирует уже поставленный кадр', async () => {
    ...
    const { useSpring } = await import('../src/qwik/index.js');
    const vc = makeVirtualClock();
    const spring = useSpring(0, SPRING, 'instant', vc.requestFrame);

    spring.target.value = 100;
    fireTrackedTasks();
    reduced = true;
    spring.target.value = 200;
    fireTrackedTasks();
  ...
  it('full→reduce гасит полёт, даже если новая цель равна текущему кадру', async () => {
    let reduced = false;
    (globalThis as { window?: unknown }).window = {
      matchMedia: () => ({ get matches() { return reduced; } }),
    };
