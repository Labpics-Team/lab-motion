...
describe('differential: douglasPeuckerVertical — kept-индексы идентичны прежним', () => {
  ...
  it('seeded-фазз по ВАЛИДНЫМ пружинам: kept-индексы new≡old на домене компилятора', () => {
    // Закрывает несущий клейм КЛАССОМ, не 180 примерами: свип по всему валидному
    // пространству параметров (как фазз группы 1), а не по горсти режимов. Если
    // замена (dy·Δx)/dx→(dy/dx)·Δx когда-нибудь перекинет argmax на РЕАЛЬНОЙ
    // пружине — тест это вскроет; пока держится ⇒ клейм байт-идентичного выхода
    // сегментатора забетонирован по домену, а не заспот-чекан.
    ...
    for (let n = 0; n < 600; n++) {
      ...
      const T = settleTimeUpperBound(p);
      if (!Number.isFinite(T) || T <= 0) continue; // неоседающие/вырожденные — мимо
      const count = [17, 33, 65, 129, 257][n % 5]!; // те же сетки, что подаёт компилятор
      const intervals = count - 1;
      const xs: number[] = [];
      const ys: number[] = [];
      for (let i = 0; i < count; i++) {
      ...
      const eps = [1e-4, 5e-4, 1e-3, 2e-3, 5e-3, 1e-2][n % 6]!;
      expect(douglasPeuckerVertical(xs, ys, eps)).toEqual(
        douglasPeuckerVerticalOld(xs, ys, eps),
      );
      checks++;
    }
    // Санити: свип реально прогонял сравнения (не выродился в сплошной continue).
