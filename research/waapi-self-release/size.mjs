...
function protect(b,c,path='root'){
 ...
 for(const [k,v] of Object.entries(b)){
  if(typeof v==='number'&&/(?:raw|gz|br)Bytes$/i.test(k)&&c[k]>v)result.violations.push({path:`${path}.${k}`,base:v,candidate:c[k],delta:c[k]-v});
...
for(const file of files){const b=readFileSync(`${base}/dist/${file}`),c=readFileSync(`${candidate}/dist/${file}`);if(b.equals(c))continue;
 const cost=x=>({rawBytes:x.length,gzBytes:compression.canonicalGzip(x).length,brBytes:compression.observationalBrotli(x).length});const row={file,base:cost(b),candidate:cost(c)};protect(row.base,row.candidate,`dist.${file}`);result.dist.push(row);}
result.pass=result.violations.length===0;
writeFileSync(out,JSON.stringify(result,null,2));console.log(JSON.stringify({pass:result.pass,violations:result.violations,changed:result.dist,entries:result.base.entries.rows.length,consumers:result.base.consumers.length}));if(!result.pass)process.exitCode=2;
