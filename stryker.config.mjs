 ...
 * По итогам замера двумя самыми слабыми были sliding-window.ts (71.15, 6
 * мутантов без покрытия) и surface-batch.ts (71.98, 2 без покрытия) — оба
 * закрыты в тот же день:
 *   sliding-window 71.15 → 92.31 (без покрытия 6 → 0) — ветка компакции
 *   ленты сэмплов, единственная защита трекеров скорости от линейного роста;
 *   surface-batch  71.98 → 77.78 (без покрытия 2 → 0) — окно реентрантного
 *   host при подписке и закон «компакция дыр не идёт внутри кадра».
 *
 * ОСТАЁТСЯ ОЧЕРЕДЬЮ РАБОТЫ: projection/geometry.ts 79.54 (80 выживших, все
 * покрыты — это слабость АССЕРТОВ, а не мёртвый код) и оставшиеся 46 выживших
 * surface-batch.ts. Это не приемлемое состояние, а названный долг.
 *
