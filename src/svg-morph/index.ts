 ...
 *
 * Скоуп-предел: ОТКРЫТЫЕ пути с совпадающей структурой морфятся
 * покомпонентно как есть, без пере-выравнивания — у открытого пути нет
 * winding'а, а реверс менял бы семантику «какая точка стартует анимацию»;
 * направление соответствия задаёт потребитель порядком точек.
 *
