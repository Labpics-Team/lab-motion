 ...
 *   через createMotionPath (subdivision = samples), морф идёт по полилинии.
 *   Для пары замкнутых путей стартовые точки ВЫРАВНИВАЮТСЯ: перебор
 *   циклического сдвига И направления обхода (пути из разных редакторов
 *   часто с противоположным winding), минимизируется Σd² — иначе морф
