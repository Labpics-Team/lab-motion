 ...
 *
 * Что ОТВЕРГНУТО с доказательствами (контрфакты, НЕ реализованы —
 * см. docs/architecture.md «Отвергнутые пути»):
...
export interface CompositorPlanOptions {
  ...
  readonly tolerance?: number;
  /** Макс. ошибка реконструкции в единицах numeric from/to до format. */
 ...
 * Полная таблица «тир → поведение → что теряем» — docs/compositor.md
 * «Fallback-матрица».
 *
