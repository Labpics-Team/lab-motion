 ...
 *     выбрана сознательно: sqrt — одна FPU-операция на канал на кадр.
 *     ЦЕНА РАЗМЕНА (замер по всем 256×256 парам каналов при t=0.5, аудит
 *     2026-07-25): отличие от точной EOTF ≤ 7.33/255, худшая пара — (9, 255),
 *     то есть очень тёмный канал против максимального; каноничный red→blue
 *     даёт 180.31 против 187.52, разницу 7.21. Прежняя запись «≤ 3/255» была
 *     занижена в 2.4 раза — число не проверялось ничем. Порог одномоментной
 *     различимости для ДВИЖУЩЕГОСЯ градиента это по-прежнему не превышает,
 *     поэтому размен остаётся; но он ДЕКЛАРИРОВАННЫЙ и теперь измеренный, а
 *     не «точный linear-light». Пин: test/value-color-gamma-budget.test.ts.
