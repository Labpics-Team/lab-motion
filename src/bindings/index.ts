...
export function createMotionBinding<Model, Goals extends MotionBindingGoals>(
  ...
  if (typeof project !== 'function' || !record(targets)) throw new MotionParamError('LM179');
  let roles = Object.keys(targets);
  if (!roles.length || roles.length > MAX_ITEMS) throw new MotionParamError('LM179');
  let ports = roles.map(role => targets[role]!);
  if (ports.some(port => typeof port !== 'function')) throw new MotionParamError('LM179');
  ...
  function snapshot(value: unknown): readonly MotionBindingGoal[] | undefined {
    if (!record(value)) throw new MotionParamError('LM180');
    ...
    for (let i = 0; i < roles.length; i++) {
      ...
      if (state !== 'active') return;
      if (!record(input)) throw new MotionParamError('LM180');
      const names = Object.keys(input);
      if (state !== 'active') return;
      count += names.length;
      if (!names.length || count > MAX_ITEMS) throw new MotionParamError('LM180');
  ...
  function capture(value: MotionBindingHandle | void): Effect | undefined {
    ...
    if (value === null || (typeof value !== 'object' && typeof value !== 'function')) {
      throw new MotionParamError('LM179');
    }
    const stop = value.cancel;
    if (typeof stop !== 'function') throw new MotionParamError('LM179');
  ...
  return {
    update(model): void {
      if (state !== 'active') return;
      if (projecting) throw new MotionParamError('LM181');
      projecting = true;
