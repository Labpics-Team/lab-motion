...
## Стирание полного фасада (#240)

Понижается не только `./nano`. Вызовы **полного** `./animate` компилятор
понижает **автоматически**: установка плагина и есть согласие — выбирать на
каждый вызов вам не нужно.

```ts
import { animate } from '@labpics/motion/animate';

export function reveal(el: HTMLElement): void {
  animate(el, { x: 240, opacity: 1 });   // уедет в артефакт, фасад — из бандла
}
```

Понижённый вызов ведёт себя как фасад, а не «почти как»:

- стартует из **текущего положения элемента** — реестр прогонов, живой стиль,
  identity для transform (тот же каскад, что у фасада), включая точную позицию
  при прерывании на лету;
- та же кривая (артефакт `compositor/curve`), тот же тайминг, та же политика
  `prefers-reduced-motion`, те же явные стопы на WebKit, который не исполняет
  CSS `linear()`.

Единственное отличие: при прерывании **на лету** понижённый путь сохраняет
непрерывность положения, но не скорости — для подхвата скорости нужен солвер,
которого в бандле уже нет. Нужен именно он — пометьте вызов тем же маркером,
что и в nano-грамматике:

```ts
/* @motion-runtime */ animate(el, { x: 0 });   // останется полным фасадом
```

Не понижаются (и остаются фасадом молча, сборка не падает): tween-режим
(`duration`/`ease`), треки из трёх и более стопов, CSS- и var-каналы, цвета,
вызовы с используемым результатом, нестатические значения. Понижаемая
грамматика: transform-шортхенды (`x`, `y`, `scale`, `scaleX`, `scaleY`,
`rotate`, `skewX`, `skewY`) и `opacity`; конечные числовые литералы (включая
унарный минус) и пары `[from, to]`; опции `spring`, `delay`, `stagger`.

### Сколько фасада осталось в бандле и почему

```ts
motionCompiler({
  onBudget: ({ lowered, facadeCallsKept }) => {
    console.log(`понижено: ${lowered}, осталось на фасаде: ${facadeCallsKept}`);
  },
});
```

`facadeCallsKept` равен нулю ⇒ фасад вытряхивается целиком. Нужна гарантия —
`strict: true` превращает любой непониженный вызов в ошибку сборки с файлом,
позицией и причиной.
