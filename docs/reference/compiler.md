...
- Тривиа-зоны вызова верифицируются **побайтно**: между частями вызова ровно `(`, `,`, `)` с пробельными символами. Скобки вокруг callee/target (`(animate)(el, …)`), комментарии внутри вызова и прочая экзотика — отказ.
