...
| `ease` | функция `t∈[0,1] → прогресс` либо массив функций | `easing.standard` — `cubic-bezier(0.2, 0, 0, 1)` | Изинг tween — только ФУНКЦИИ (готовые кривые — `./easing`). CSS-строки вида `'ease-out'` здесь НЕ принимаются — строковая грамматика есть только у `./nano`. Не-функция (или элемент массива) — `LM138`. Массив (#205) — per-segment изинги N-keyframe вызова, длина ровно `N − 1` (пустой массив либо неверная длина — `LM169`). Scalar ease на треке применяется К КАЖДОМУ сегменту |
