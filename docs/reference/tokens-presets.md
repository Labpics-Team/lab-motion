...
Валидирует и нормализует `PresetSpec` — **единственная точка валидации субпутя**: всё структурно невалидное падает здесь `MotionParamError` (по-русски, префикс `presets:`), а не превращается тихо в `NaN` на кадре. Возвращает `CompiledPreset` — readonly-форму для горячего сэмплирования, брендированную маркером `__compiledPreset: true` (защита от подсовывания сырой спеки).
