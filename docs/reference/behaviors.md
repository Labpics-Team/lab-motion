...
- `pointerDown` во время доводки **перехватывает** её: активный цикл гасится. У sheet/dismiss/carousel текущая скорость доводки при этом наследуется прайором трекера — немедленный повторный release продолжает движение без излома (C¹-pickup). У pull-to-refresh перехват гасит цикл **без** наследования скорости (и не принимается вовсе, пока `pending: true` — pending владеет позицией): немедленный повторный release стартует с нулевой скоростью.
...
- **C¹-непрерывность (sheet/dismiss/carousel).** Release наследует скорость трекера (нормировка на диапазон), перехват `pointerDown` — скорость активной доводки через прайор трекера: производная непрерывна на стыках follow|release и release|follow. У pull-to-refresh перехват `pointerDown` скорость активной доводки не наследует — C¹-pickup на стыке release|follow там отсутствует.
