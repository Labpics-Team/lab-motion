...
диапазона — разность `to − from` не конечна, т.е.
`|to − from| > Number.MAX_VALUE` (проверяется именно разность, не сумма
модулей: при `from = 1.7e308`, `to = 1e308` разность конечна и драйвер
анимирует обычным циклом); reduced-motion. Во всех трёх случаях — один
`onStep(to)` и разрешённый Promise.
