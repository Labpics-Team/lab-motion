...
- Путь без сегментов (одна команда `M`, в т.ч. с `Z`: `'M 10 10'`, `'M 10 10 Z'`) не порождает ни одного кубика — таблица сэмплов пуста, и `at` возвращает `{ x: 0, y: 0, angle: 0 }` (не стартовую точку). Путь с хотя бы одним вырожденным сегментом нулевой длины (например `'M 10 10 L 10 10'`) → `at` возвращает стартовую точку с `angle: 0`.
