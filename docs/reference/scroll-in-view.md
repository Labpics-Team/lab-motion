...
- число — доля размера, клампится в [0, 1] (`size > 0 && visible > 0 && visible >= amount × size`); неконечное число → дефолт `'some'`. Доля `0` означает «любой видимый пиксель», то есть ровно то же, что `'some'`, — а не «видимо всегда».
