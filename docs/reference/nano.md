...
- `finished: Promise<Animation[]>` — `Promise.all` по всем целям. На `finish` каждой анимации финал фиксируется `commitStyles()` и анимация снимается `cancel()` — **микротаском после рассылки события**: пользовательские `finish`-listeners и `onfinish` (библиотека его не занимает) видят анимацию ещё в состоянии `'finished'`, а если хендлер перезапустил её (`play()`), чистка уступает новому циклу и промис следует за ним. На платформе без `commitStyles` финал удерживает `fill: 'both'`. Резолвится массивом всех `Animation` после фиксации финала; отклоняется, если `finished` текущего цикла любой из анимаций отклонился (пользовательский `cancel()` — в том числе отменивший replay).
