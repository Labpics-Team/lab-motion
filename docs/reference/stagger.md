...
- **ST1. Финитность.** Каждая ВОЗВРАЩЁННАЯ задержка — конечное неотрицательное число. `NaN`/`Infinity` в поэлементных входах (`gap`, возврат `easing`) → задержка клампится до `0`; неконечный `count` задержек не порождает вовсе — возвращается `[]` (см. ST5).
