...
- **ST1. Финитность.** Каждая ВОЗВРАЩЁННАЯ задержка — конечное неотрицательное число. Неконечные входы обрабатываются по-разному: неконечный (или отрицательный) `gap` заменяется дефолтом `50` — `stagger(3, { gap: NaN })` → `[0, 50, 100]`, а не нули; неконечный `count` задержек не порождает вовсе — возвращается `[]` (см. ST5); до `0` клампится только неконечный возврат `easing` и отрицательная/неконечная производная задержка.
