...
Пружина + `from`/`to`/`property` → исполнимый план `Element.animate()`. Длительность выводится из аналитического закона оседания пружины и, если нужно, продлевается так, чтобы остаток `|p(T) − 1|` уместился в половину бюджета реконструкции: последний узел плана равен ровно `1`, поэтому этот остаток — такая же ошибка, как и всё остальное в бюджете. Медленные пружины (`ω₀ < 2·CONVERGENCE_THRESHOLD/tolerance`, при дефолте — `ω₀ < 4`) получают из-за этого более длинный план; у остальных длительность равна времени оседания без изменений. `maxValueError` переводится в эффективную толерантность `min(tolerance, maxValueError/|to−from|)` (строже двух бюджетов); вырожденный span (`|to−from| ≈ 0`) деление не выполняет. SSR-safe: capability-проба формы кривой (WebKit vs `linear()`) не обращается к DOM и fail-closed вне браузера.
...
Конструктор бросает: `LM088`–`LM091` (пружина), `LM010` (`property`), `LM009` (`from`/`to` не конечные), `LM014` (`tolerance`), `LM013` (`delay` не конечное или `< 0`). `retarget`/`handoffToLive` бросают `LM009` на неконечную цель (`LM015` — код только свободной функции `handoffToLive(opts)`, см. ниже); compositor-компиляция может транзитом дать `LM016`.
