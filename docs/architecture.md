...
4. **Потребители**: 9 биндингов (optional peer) поверх шедулера — либо пружина
   ...
   `Element.animate` прямо из солвера ([compositor.md](compositor.md)).
...
(interior error ≤ tolerance, CSS/WebKit parity, effect-space continuity вне
kink; `test/perf-hot-path.test.ts`: кадры до сходимости = вызовы солвера,
машинонезависимо).
