...
Для диагностики lowering без отдельного runtime-инспектора используется тот же
Vite-плагин и тот же `dist`, что в приёмочном гейте:
...
в SSR/browser/production graph.
