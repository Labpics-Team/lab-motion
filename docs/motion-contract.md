...
| **app membership** | Находится ли сущность/представление в текущей модели приложения и семантическом дереве. | Приложение. Presence получает desired intent, но не владеет app model или DOM membership. | `current` |
| **visual lifetime** | Сколько старый/новый pixels-representation остаётся видимым ради перехода. Может пережить app membership. | `./smart` владеет своими ghosts; Future Layout владеет snapshot-плоскостями; для presence representation остаётся у caller/renderer, а presence владеет только принятыми controls и completion boundary. | `current` |
...
Presence поэтому не является владельцем пиксельного представления: `createPresenceTransition`
координирует desired intent и переданные ему animation controls, но не создаёт, не удерживает и
не удаляет DOM. Видимость caller-owned representation до terminal boundary остаётся обязанностью
исполнителя/consumer, который эти controls создал.
...
|---|---|---|---|---|
| **Карточка ↔ детали** | App state выбирает сущность; stable key связывает представления; projection/smart ведёт geometry; binding ведёт visual roles, presence при необходимости координирует caller-owned enter/exit controls. | Новый app DOM/model может быть уже committed, пока старый pixels-representation живёт как snapshot/ghost; identity переживает пересоздание узла; focus boundary задаётся consumer. | Считать DOM object identity сущностью; держать старое представление app-member только потому, что оно ещё видно; позволить ghost получить focus «по факту DOM». | `current` + focus `current-gap` |
| **Диалог/панель ↔ источник** | App владеет desired presence; `createPresenceTransition` владеет принятыми enter/exit controls и completion boundary, но не DOM/pixels representation; trigger/dialog a11y owner владеет focus transfer. | Повтор того же presence state идемпотентен; встречный intent создаёт successor до cleanup прежних controls; `onGone` является terminal boundary, после которой caller может скрыть/удалить representation, а не app-state rollback. | Кодировать повторный submit/Escape как повтор state target; считать `destroy/cancel` удалением DOM или возвратом focus. | `current` + focus boundary consumer-owned |
...
- presence controls ownership/terminal cleanup: `test/presence-transition*.test.ts`;
