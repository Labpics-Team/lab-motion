...
Node ≥ 22.12 (пол задан `require(esm)`: поставка одноформатная — ESM, и
`require()` из CJS работает без флага начиная с этой версии). Если вы
потребляете пакет из **CommonJS на TypeScript** с `module: nodenext`, нужен
tsc ≥ 5.8 — 5.7 отвергает такой импорт ошибкой `TS1479`. Для ESM-потребителя
ограничения на версию tsc нет.
Runtime-зависимостей нет; фреймворк нужен только соответствующему
...
### Единицы времени: два яруса

Пакет живёт в двух системах единиц, и это следствие архитектуры, а не
небрежность. Правило простое — **смотрите, кто исполняет анимацию**:

| Ярус | Субпути | Единица | Почему |
| --- | --- | --- | --- |
| Фасадный | `./animate`, `./nano`, `./compositor`, `./compositor/stagger`, `./stagger`, `./compiler` | **миллисекунды** | тайминги уходят прямо в `Element.animate()` и CSS — конвенция WAAPI |
| Физический | `./spring`, `./timeline`, `./keyframes`, `./presets`, `./auto`, `./gestures`, `./scroll`, `./behaviors`, `./waapi` (вход компилятора) | **секунды** | одна система единиц с солвером пружины: `stiffness`/`damping` заданы в СИ |
...
animate('.card', { x: 100 }, { duration: 300 });          // 300 МИЛЛИСЕКУНД
fromBounce({ duration: 0.3, bounce: 0.2 });               // 0.3 СЕКУНДЫ
createTimeline({                                          // 0.3 СЕКУНДЫ
  segments: [{ from: 0, to: 1, duration: 0.3 }],
});
```

Цена ошибки — ровно ×1000, и **исключения не будет**: `duration: 300` в
таймлайне физически валиден, просто это пять минут. Поэтому единица объявлена
машинно — тегом `@unit s` | `@unit ms` в JSDoc КАЖДОГО публичного поля со
временем (видно в подсказке IDE в момент набора значения); тег и полная карта
единиц пришпилены контракт-тестом `test/time-units-contract.test.ts`.

Мигрируете с Framer/Motion (там секунды везде)? На фасадном ярусе умножайте
на 1000, на физическом — оставляйте как есть.
