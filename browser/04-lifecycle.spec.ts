...
test('cancel(): finished резолвится, onComplete НЕ зовётся, значение удержано', async ({
  ...
  const r = await page.evaluate(async () => {
    ...
    // Продвинули виртуальные часы и прервали.
    clock = 120;
    controls.cancel();
...
test('естественное завершение: setTimer-шов → onComplete один раз, finished резолвится', async ({
  ...
  const r = await page.evaluate(async () => {
    ...
    // Дать пружине «осесть»: выстрелить запланированный settle-таймер.
    for (const cb of [...timers.values()]) cb();
...
test('stale-finish: перехват (supersede) снимает таймер старого прогона', async ({ page }) => {
  const r = await page.evaluate(async () => {
    ...
    // Выстрелить ВСЕ оставшиеся таймеры: старый (перехваченный) не должен ожить.
    for (const cb of [...timers.values()]) cb();
...
test('retarget: C¹-снимок совпадает с readCompositorSpring в момент перехвата', async ({
  ...
  const r = await page.evaluate(async () => {
    ...
    const elapsedMs = 90;
    clock = elapsedMs;
    const predicted = compositor.readCompositorSpring(spring, {
      from,
      to,
      t: elapsedMs / 1000,
    });
    ...
    cs.destroy();
    el.remove();
    return { tier, predictedValue: predicted.value, valueAtRetarget };
  });

  expect(r.tier).toBe('compositor');
  // Позиция на стыке ретаргета непрерывна (C⁰): from' = аналитический снимок.
  expect(Math.abs(r.valueAtRetarget - r.predictedValue)).toBeLessThanOrEqual(0.001);
});
