...
test('cancel(): finished резолвится, onComplete НЕ зовётся, значение удержано', async ({
  ...
  const r = await page.evaluate(async () => {
    ...
    // Нативный currentTime — источник истины WAAPI; JS now используется только
    // когда host его не отдаёт. Двигаем именно effect, иначе cancel до первого
    // браузерного кадра корректно удержал бы исходный `none`.
    clock = 120;
