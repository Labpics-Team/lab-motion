...
/**
 * Доменный differential проверяет старый построитель против нового без дублей.
 * Здесь замыкается другая граница: реальные движки обязаны одинаково исполнять
 * полученные Q/A с тождественными interior-stops и без них. Не заменяет pack-
 * приёмку: в браузер передаются реальные CSS-строки, а не JS-модель WAAPI.
 */
for (const [name, damping] of [['default', 26], ['under', 9]] as const) {
  for (const [from, to] of [[240, 360], [360, 240]] as const) {
    test(`${name} ${from}→${to}: тождественные узлы Q/A не меняют native интерполяцию`, async ({ page }) => {
      ...
      const curves = [artifact.reciprocalEasing, artifact.blendEasing].map((css) => {
        ...
        return {
          css,
          duplicate: `linear(${duplicate.join(', ')})`,
          changed: `linear(${changed.join(', ')})`,
          positions,
          witness: positions[middle]!,
        };
      });
      const results = await page.evaluate((curves) => curves.map((curve) => {
        ...
        try {
          ...
          for (const percent of times) {
            ...
            if (getComputedStyle(elements[0]!).transform !== getComputedStyle(elements[1]!).transform) {
              mismatches++;
            }
          }
          for (const animation of animations) animation.currentTime = curve.witness * 10;
          const controlDetected = getComputedStyle(elements[0]!).transform
            !== getComputedStyle(elements[2]!).transform;
