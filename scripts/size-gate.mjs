...
export const BESPOKE_SUBPATH_GATES = {
  ...
  // (~4% люфт, ОТ ФАКТА).
  // 2026-07-23 (#240 facade-erasure): 6990 → 9700 (~4% от факта 9,29 KB gz).
  // Адаптер несёт вторую грамматику — понижение полного ./animate: словарь
  // transform-шортхендов с identity-инференсом from, пары [from, to], опции
  // spring/delay/stagger, прагма @lm-oneshot, слияние правок двух грамматик в
  // один проход парсинга и артефакт из compositor/curve (та же кривая, что у
  // фасадного tier-0 — отсюда вес: build-side тянет сегментер). Build-tool
  // entry, браузеру НЕ поставляется: цена платится один раз в dev-зависимости.
  './compiler/vite': 9700,
  ...
  // 2026-07-23 (#240 facade-erasure): 390 → 1250. Модуль теперь несёт ВТОРОЙ,
  // независимо вытряхиваемый исполнитель — animateFacadeCompiled (понижённый
  // полный ./animate): кадры двух режимов (2-кадровый linear() и явные стопы
  // для WebKit, который linear() не исполняет), reduced-политика фасада и
  // общий SSOT buildTransform (импорт, не копия). Рост — shipped-размер файла;
  // потребитель платит только за использованный экспорт, что запечатано
  // exact-ратчетом леджера ×1 = 883 B gz (бит-в-бит прежний, см.
  // compiler-acceptance) и новым ратчетом стирания фасада там же.
  // Решение Даниила: парадигма «нулевая библиотека» (12 475 → 1 524 B gz
  // у потребителя, −87.8%) стоит килобайта в build-tool субпути.
  './compiler/runtime': 1250,
