...
export const BESPOKE_SUBPATH_GATES = {
  ...
  // 2026-07-23: факт 386 gz — ревью-фикс контракта onfinish: чистка
  // commitStyles/cancel отложена микротаском ПОСЛЕ рассылки finish (пользователь-
  // ские listeners видят finished-состояние) + guard по playState (replay из
  // хендлера не затаптывается). Рост частично оплачен шейвами (?? для r,
  // прямой return). Паритет с nano-хвостом сохранён (C4).
  './compiler/runtime': 386,
