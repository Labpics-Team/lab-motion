...
const EXACT_FIELDS = Object.freeze({
  ...
  // `imports` отсутствует намеренно. Алиас #frame существовал ради CJS-ветки:
  // .cjs-файлы несли его в рантайме и разрешали через package.json. ESM-ветка
  // всегда переписывалась в относительный путь (браузер/CDN не читает
  // package.json), поэтому после снятия CJS ни один отгружаемый байт алиас не
  // упоминает — это проверяет гейт «browser/CDN ESM» в pack-smoke. Объявлять
  // его дальше значило бы держать в контракте запись, которую ничто не
  // использует.
  files: [
    'dist',
    'docs/errors.md',
