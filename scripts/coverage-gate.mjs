...
const AREA_FLOORS = {
  ...
  // lit: 2026-07-24, две записи за вечер.
  //   (1) пол был откалиброван по локали (ветви 88.46 %), в CI факт
  //       оказался 84.62 % — ОДНА ветвь исполняется на Node 22 и не
  //       исполняется на Node 24. Авторитетна среда CI.
  //   (2) долг закрыт по существу: три ветви, не исполнявшиеся НИ ОДНИМ
  //       тестом (window без matchMedia, reconnect элемента, повторная
  //       регистрация тега), покрыты test/lit-runtime-branches.test.ts —
  //       локальный факт ветвей 96.15 % (25/26). Пол 91.5 оставляет запас
  //       на ту самую Node-зависимую ветвь и всё равно вдвое строже
  //       прежнего 83.9.
  lit: { lines: 95.0, branches: 91.5 },
