...
const AREA_FLOORS = {
  ...
  // lit: 2026-07-24 → 25, три записи; долг закрыт полностью.
  //   (1) пол был откалиброван по локали (ветви 88.46 %), в CI 84.62 % —
  //       расхождение сред. Авторитетна среда CI.
  //   (2) покрыты три ветви, не исполнявшиеся ни одним тестом.
  //   (3) расхождение Node 22/24 локализовано и УСТРАНЕНО: причиной была
  //       ветвь `if (!this._motion)` в _applyStyle (element.ts) —
  //       на Node 22 её случайно задевал порядок микрозадач Lit, на
  //       Node 24 не задевал ни разу. Теперь она вызывается явно.
  //       Заодно вскрылось, что jsdom этой версии НЕ реализует
  //       window.matchMedia, поэтому ambient-ветка reduced-motion —
  //       та, что работает в каждом реальном браузере, — не исполнялась
  //       ни одним unit-тестом. Покрыта.
  //       Факт на ОБЕИХ версиях Node: ветви 100 %, строки 97.87 %.
  lit: { lines: 97.1, branches: 99.2 },
