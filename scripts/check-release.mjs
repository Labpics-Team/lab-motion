...
const releaseDateInput = process.argv[3] ?? process.env.RELEASE_DATE;
...
if (!releaseDateInput) fail('передайте UTC-дату release intent аргументом или через RELEASE_DATE');
...
try {
  const releaseDate = releaseDateInput === '--validate-stored-date'
    ? readReleaseChangelogDate(changelog, pkg.version)
    : releaseDateInput;
