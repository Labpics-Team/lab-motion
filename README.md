...
1. **Всё — субпути.** Корневой экспорт + 35 субпутей (36 входов `exports` в
...
| `…/animate/mini` | **Лёгкий срез (≤ 5 KB gz)** поверх адаптерного реестра кодеков/адаптеров: transform-шортхенды, `opacity`, CSS-переменные, spring/tween в едином прогресс-клоке, `delay`/`stagger`, контролы, reduced-motion снап. Расширение — **регистрацией кодека**, не ростом switch (цвет/SVG-атрибут/plain-object — `createFullRegistry`). Граница поставки: mini не тянет `./value` (цвета) и compositor-компилятор. Compositor-offload (WAAPI) в mini не входит (не помещается под 5 KB) — он в `./animate`. Таблица миграции с Motion JS / Anime.js — ниже |
...
| Импорт | Что даёт |
|---|---|
| `…/react` | `useSpring`, `useMotionValue`, `useMotionStyle` (effect-binding: пишет в `style` через ref без render на кадр — аналог `vMotion`), `useReducedMotion` (реактивное системное `prefers-reduced-motion`, hydration-safe), `useVelocity`/`useTransform` (render-value деривации поверх `MotionValue`) |
...
Поддержка выражена в **возможностях среды, а не в UA** — никакого UA-снифинга.
Детекция (`resolveCompositorTier` / `supportsLinearEasing` / `supportsWaapi`)
выбирает движок по фактам среды; чего среда не умеет — деградирует прозрачно
(см. «Fallback-матрица»), а не притворяется.

| Среда | Статус | Основание |
|---|---|---|
| Chromium (Chrome/Edge), evergreen | Полный `compositor`-путь | WAAPI + CSS `linear()` |
| Firefox, evergreen | Полный `compositor`-путь | WAAPI + CSS `linear()` |
| WebKit (Safari), evergreen | Полный `compositor`-путь | WAAPI + CSS `linear()` |
| Движки до CSS `linear()` (Baseline 12.2023) | `waapi-no-linear` → живой rAF | `linear()` не донёс бы кривую off-main |
| Без `Element.animate` | `raf` → живой rAF | нет WAAPI |
| SSR / Node ≥ 18 | `ssr` → импорт SSR-safe, кадры не рисуются | нет DOM; см. `pack:compat` |
...
- Compositor-путь — для автономных переходов и release-фазы; **покадровый**
  `retarget` (gesture-follow) — антипаттерн (follow-фаза жестов живёт на main-потоке,
  `./gestures` + `MotionValue`).

**Conformance-слой.** Реальное поведение движков сверяется с аналитическими
предсказаниями солвера в `browser/*.spec.ts` (Playwright, три движка). Локально:
...
Consumer-контракт тарбола (ESM/CJS/TypeScript/Vite/SSR + Node ≥ 18): `pnpm pack:compat`.
...
Расширенная поверхность (`keyframes`-массивы, per-property `transition`, `repeat`,
function-values, sequences, цвет/SVG-атрибут/plain-object адаптеры) — субпуть
`./animate` и `createFullRegistry` (регистрация кодека, не рост switch).
...
`./animate` 10700, `./animate/mini` 5120 (headline «≤ 5 KB» эпика).
