...
Для CJS-потребителя **на TypeScript** дополнительно нужен tsc ≥ 5.8 при
`module: nodenext`: 5.7 отвергает `require()` ESM-цели с `TS1479` (замер на
том же тарболе — 5.7.3 красный, 5.8.3 и 5.9.3 зелёные). ESM-потребителя это
не касается.
